...
describe('Financial API Integration Tests', () => {
  ...
  describe('Stock Data API Integration', () => {
    test('should fetch Korean stock data successfully', async () => {
      ...
      (financialService.getStockData as jest.Mock).mockResolvedValueOnce(
        MOCK_STOCK_DATA['005930.KS']
      );
    ...
    test('should handle API rate limiting with retry', async () => {
      ...
      (financialService.getStockData as jest.Mock).mockImplementationOnce(
        async () => {
    ...
    test('should handle invalid symbol gracefully', async () => {
      ...
      (financialService.getStockData as jest.Mock).mockRejectedValueOnce(
        new Error('Symbol not found')
      );

      await expect(financialService.getStockData('INVALID')).rejects.toThrow(
        'Symbol not found'
      );
    });
    ...
    test('should handle network timeout', async () => {
      ...
      await expect(financialService.getStockData('AAPL')).rejects.toThrow(
        'Network timeout'
      );
    });
  ...
  describe('Forex Data API Integration', () => {
    test('should fetch forex rates successfully', async () => {
      ...
      (global.fetch as jest.Mock).mockResolvedValueOnce(mockResponse);
      (financialService.getForexData as jest.Mock).mockResolvedValueOnce(
        MOCK_FOREX_DATA['USD/KRW']
      );
    ...
    test('should handle forex API failures with fallback', async () => {
      ...
      // Test retry logic
      const result = await financialService
        .getForexData('USD', 'KRW')
        .catch(async () => {
          // Simulate fallback

...
class PerformanceTestRunner {
  ...
  async runLoadTest(scenario = 'default') {
    ...
    const outputFile = path.join(
      this.outputDir,
      `load-test-${scenario}-${timestamp}.json`
    );
  ...
  extractSummaryMetrics(data) {
    ...
    return {
      ...
      errorRate: summary.errors
        ? ((summary.errors / summary.requestsCompleted) * 100).toFixed(2)
        : 0,
    ...
    phases.forEach(phase => {
      if (phase.customStats) {
        ...
        if (phase.customStats.financial_api_response_time) {
          financialApiData.averageStockResponseTime =
            phase.customStats.financial_api_response_time.mean || 0;
        }
      }
    });
  ...
  generateRecommendations(analysis) {
    ...
    if (performance.financialApiMetrics.averageStockResponseTime > 1000) {
      recommendations.push({
        ...
        description:
          'Financial API response times are impacting user experience',
        actions: [
  ...
  async runFinancialStressTest() {
    ...
    const outputFile = path.join(
      this.outputDir,
      `financial-stress-${timestamp}.json`
    );
...
`;
  ...
  aggregatePerformanceMetrics() {
    const allAnalyses = this.testResults
    ...
    return {
      averageResponseTime: this.calculateAverage(
        allAnalyses.map(a => a.summary.averageResponseTime)
      ),
      averageP95: this.calculateAverage(
        allAnalyses.map(a => a.summary.p95ResponseTime)
      ),
      averageErrorRate: this.calculateAverage(
        allAnalyses.map(a => parseFloat(a.summary.errorRate))
      ),
      averageThroughput: this.calculateAverage(
        allAnalyses.map(a => a.summary.requestRate)
      ),
    };
  }

...
describe('Tax Calculations', () => {
  ...
  describe('Capital Gains Calculation', () => {
    ...
    test('should handle complex FIFO scenarios', () => {
      const complexTransactions: Transaction[] = [
        { id: '1', type: 'BUY', symbol: 'TEST', shares: 100, price: 50, amount: 5000, fees: 5, currency: 'USD', date: '2024-01-01', isKorean: false },
        { id: '2', type: 'BUY', symbol: 'TEST', shares: 200, price: 60, amount: 12000, fees: 12, currency: 'USD', date: '2024-02-01', isKorean: false },
        { id: '3', type: 'SELL', symbol: 'TEST', shares: 150, price: 70, amount: 10500, fees: 10.5, currency: 'USD', date: '2024-03-01', isKorean: false }
      ]
    ...
    test('should handle losses correctly', () => {
      const lossTransactions: Transaction[] = [
        { id: '1', type: 'BUY', symbol: 'LOSS', shares: 100, price: 100, amount: 10000, fees: 10, currency: 'USD', date: '2024-01-01', isKorean: false },
        { id: '2', type: 'SELL', symbol: 'LOSS', shares: 100, price: 80, amount: 8000, fees: 8, currency: 'USD', date: '2024-06-01', isKorean: false }
      ]
    ...
    test('should handle insufficient shares for sale', () => {
      const insufficientTransactions: Transaction[] = [
        { id: '1', type: 'BUY', symbol: 'INSUF', shares: 50, price: 100, amount: 5000, fees: 5, currency: 'USD', date: '2024-01-01', isKorean: false },
        { id: '2', type: 'SELL', symbol: 'INSUF', shares: 100, price: 120, amount: 12000, fees: 12, currency: 'USD', date: '2024-06-01', isKorean: false }
      ]
  ...
  describe('Dividend Income Calculation', () => {
    ...
    test('should handle only domestic dividends', () => {
      const domesticOnly = sampleTransactions.filter(t => t.isKorean || t.type !== 'DIVIDEND')
    ...
    test('should handle only foreign dividends', () => {
      const foreignOnly = sampleTransactions.filter(t => !t.isKorean && t.type === 'DIVIDEND')
  ...
  describe('Deductible Fees Calculation', () => {
    ...
    test('should handle transactions without fees', () => {
      const noFeesTransactions = sampleTransactions.map(t => ({ ...t, fees: 0 }))
  ...
  describe('Capital Gains Tax Calculation', () => {
    ...
    test('should calculate Korean stock capital gains tax', () => {
      ...
      const expectedNationalTax = capitalGains * KOREAN_TAX_RATES.capitalGains.domestic
      const expectedLocalTax = expectedNationalTax * KOREAN_TAX_RATES.localIncomeTax
    ...
    test('should calculate foreign stock capital gains tax', () => {
      ...
      const expectedNationalTax = capitalGains * KOREAN_TAX_RATES.capitalGains.foreign
      const expectedLocalTax = expectedNationalTax * KOREAN_TAX_RATES.localIncomeTax
      const expectedTotal = expectedNationalTax + expectedLocalTax
  ...
  describe('Dividend Tax Calculation', () => {
    ...
    test('should calculate dividend tax with foreign tax credit', () => {
      ...
      const result = calculateDividendTax(domesticDividends, foreignDividends, foreignWithholding)
      
      const expectedDomesticTax = domesticDividends * KOREAN_TAX_RATES.dividend.domestic
      const expectedForeignTaxBeforeCredit = foreignDividends * KOREAN_TAX_RATES.dividend.foreign
      const expectedCredit = Math.min(foreignWithholding, expectedForeignTaxBeforeCredit)
      const expectedForeignTax = Math.max(0, expectedForeignTaxBeforeCredit - expectedCredit)
      ...
      expect(result.totalTax).toBeCloseTo(expectedDomesticTax + expectedForeignTax, 2)
    })
    ...
    test('should limit foreign tax credit to Korean tax liability', () => {
      ...
      const result = calculateDividendTax(0, foreignDividends, foreignWithholding)
      
      const koreanTaxLiability = foreignDividends * KOREAN_TAX_RATES.dividend.foreign
  ...
  describe('Comprehensive Tax Liability', () => {
    ...
    test('should handle large shareholder scenario', () => {
      const normalResult = calculateTaxLiability(sampleTransactions, 0, { isLargeShareholder: false })
      const largeholderResult = calculateTaxLiability(sampleTransactions, 0, { isLargeShareholder: true })
      
      expect(largeholderResult.taxLiability.capitalGainsTax).toBeGreaterThan(normalResult.taxLiability.capitalGainsTax)
    })
    
    test('should handle different holding periods', () => {
      const shortTermResult = calculateTaxLiability(sampleTransactions, 0, { holdingPeriod: 30 })
      const longTermResult = calculateTaxLiability(sampleTransactions, 0, { holdingPeriod: 365 })
      
      // Results should be different due to different gain classifications
      expect(shortTermResult.capitalGains.shortTerm).toBeGreaterThan(longTermResult.capitalGains.shortTerm)
    })
    ...
    test('should calculate effective tax rate correctly', () => {
      ...
      const expectedEffectiveRate = grossIncome > 0 ? (result.taxLiability.totalTax / grossIncome) * 100 : 0
  ...
  describe('Quarterly Tax Calculation', () => {
    ...
    test('should calculate Q1 tax liability', () => {
      const q1Result = calculateQuarterlyTax(sampleTransactions, 1, 2024, 100000)
  ...
  describe('Tax Loss Harvesting', () => {
    ...
    const lossPositions = [
      { symbol: 'LOSS1', shares: 100, averagePrice: 100, currentPrice: 80, isKorean: true },
      { symbol: 'LOSS2', shares: 50, averagePrice: 200, currentPrice: 150, isKorean: false },
      { symbol: 'GAIN1', shares: 200, averagePrice: 50, currentPrice: 60, isKorean: true }
    ]
    ...
    test('should calculate tax savings correctly', () => {
      ...
      const expectedTaxSavings1 = expectedLoss1 * KOREAN_TAX_RATES.capitalGains.domestic
  ...
  describe('Transaction Validation', () => {
    ...
    test('should catch missing required fields', () => {
      ...
      expect(validation.errors.some(e => e.includes('Missing transaction ID'))).toBe(true)
      expect(validation.errors.some(e => e.includes('Missing symbol'))).toBe(true)
    })
    ...
    test('should catch invalid numeric values', () => {
      ...
      expect(validation.isValid).toBe(false)
      expect(validation.errors.some(e => e.includes('Invalid shares amount'))).toBe(true)
      expect(validation.errors.some(e => e.includes('Invalid price'))).toBe(true)
    })
    ...
    test('should warn about amount inconsistencies', () => {
      ...
      expect(validation.warnings.some(w => w.includes("Amount doesn't match"))).toBe(true)
    })

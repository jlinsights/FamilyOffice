...
import { FINANCIAL_SCENARIOS, scenarioManager } from '../fixtures/financial-scenarios'
...
class ComplianceTestSuite {
  ...
  private initializeComplianceRules(): void {
    this.complianceRules = [
      // Korean Financial Investment Services and Capital Markets Act
      {
        ...
        description: 'Transactions over 5 billion KRW must be reported within 24 hours',
        ...
        checkFunction: (transaction) => {
          ...
          if (transaction.currency === 'KRW' && (transaction.shares * transaction.price) > 5000000000) {
            const timeSinceTransaction = Date.now() - new Date(transaction.timestamp).getTime()
            const hoursElapsed = timeSinceTransaction / (1000 * 60 * 60)
            
            if (hoursElapsed > 24) {
              violations.push(`Large transaction not reported within 24 hours: ${hoursElapsed.toFixed(1)} hours elapsed`)
            }
          }
      ...
      // US Securities Exchange Act Rule 13d
      {
        ...
        description: 'Ownership of 5% or more of a class of securities must be disclosed',
        ...
        checkFunction: (portfolio) => {
          ...
          portfolio.positions?.forEach((position: any) => {
            ...
            const ownershipPercentage = (position.shares / mockSharesOutstanding) * 100
            
            if (ownershipPercentage >= 5 && !portfolio.hasDisclosureFilings) {
              violations.push(`${position.symbol}: ${ownershipPercentage.toFixed(2)}% ownership requires 13D filing`)
              recommendations.push(`File Schedule 13D for ${position.symbol} within 10 days`)
            }
          })
        ...
        checkFunction: (transactions) => {
          ...
          if (Array.isArray(transactions)) {
            ...
            const recentTransactions = transactions.filter((txn: any) => {
              const daysSince = (Date.now() - new Date(txn.timestamp).getTime()) / (1000 * 60 * 60 * 24)
            ...
            if (largeTransactions.length >= 3) {
              violations.push(`Potential structuring detected: ${largeTransactions.length} transactions just under $10K threshold`)
            ...
            symbols.forEach(symbol => {
              const symbolTxns = transactions.filter((txn: any) => txn.symbol === symbol)
              const buys = symbolTxns.filter((txn: any) => txn.type === 'BUY').length
              const sells = symbolTxns.filter((txn: any) => txn.type === 'SELL').length
              
              if (buys > 10 && sells > 10) {
                violations.push(`High frequency trading pattern in ${symbol}: ${buys} buys, ${sells} sells`)
                recommendations.push(`Review trading pattern for ${symbol} for potential wash trading`)
              }
            })
          }
      ...
      // GDPR Data Protection
      {
        ...
        checkFunction: (userData) => {
          ...
          if (userData.lastLoginDate) {
            const daysSinceLogin = (Date.now() - new Date(userData.lastLoginDate).getTime()) / (1000 * 60 * 60 * 24)
            
            if (daysSinceLogin > 1095) { // 3 years
              violations.push(`User data retained for ${Math.floor(daysSinceLogin)} days without activity`)
              recommendations.push('Consider data anonymization or deletion for inactive users')
            }
          }
      ...
      // Fiduciary Duty
      {
        ...
        checkFunction: (execution) => {
          ...
          if (execution.executedPrice && execution.benchmarkPrice) {
            const priceDeviation = Math.abs(execution.executedPrice - execution.benchmarkPrice) / execution.benchmarkPrice
            
            if (priceDeviation > 0.05) { // 5% deviation
              violations.push(`Execution price deviated ${(priceDeviation * 100).toFixed(2)}% from benchmark`)
              recommendations.push('Review execution venues and routing algorithms')
            }
          }
          
          if (execution.executionTime > 60000) { // 1 minute
  ...
  generateComplianceReport(): {
    ...
    const violations = allResults.filter(result => result.status === 'NON_COMPLIANT')
    const compliantCount = allResults.filter(result => result.status === 'COMPLIANT').length
    
    const summary = {
      totalChecks: allResults.length,
      compliantChecks: compliantCount,
      violations: violations.length,
      complianceRate: allResults.length > 0 ? (compliantCount / allResults.length * 100).toFixed(2) : '0.00'
    }

    const auditStatistics = {
      totalAuditEntries: this.auditLogs.length,
      uniqueUsers: new Set(this.auditLogs.map(log => log.userId)).size,
      actionBreakdown: this.auditLogs.reduce((acc, log) => {
        acc[log.action] = (acc[log.action] || 0) + 1
        return acc
      }, {} as Record<string, number>),
      resourceBreakdown: this.auditLogs.reduce((acc, log) => {
...
describe('Compliance and Audit Trail Testing', () => {
  ...
  describe('Audit Trail Functionality', () => {
    ...
    test('should log transaction execution with before/after values', () => {
      const transaction = FINANCIAL_SCENARIOS.FAMILY_OFFICE_DIVERSIFIED.transactions[0]
  ...
  describe('Korean Financial Compliance', () => {
    ...
    test('should detect large Korean transaction reporting requirements', () => {
      ...
      const results = complianceSuite.checkCompliance(largeKoreanTransaction, 'FSCMA-001')
    ...
    test('should pass compliance for timely reported transactions', () => {
      ...
      const results = complianceSuite.checkCompliance(timelyTransaction, 'FSCMA-001')
  ...
  describe('US Securities Compliance', () => {
    ...
    test('should detect beneficial ownership disclosure requirements', () => {
      ...
      const results = complianceSuite.checkCompliance(portfolioWithLargePosition, 'SEC-13D')
    ...
    test('should pass compliance for positions under 5% threshold', () => {
      ...
      const results = complianceSuite.checkCompliance(portfolioWithSmallPosition, 'SEC-13D')
  ...
  describe('Anti-Money Laundering (AML) Compliance', () => {
    ...
    test('should detect potential structuring patterns', () => {
      ...
      const results = complianceSuite.checkCompliance(suspiciousTransactions, 'AML-001')
      ...
      expect(results[0].recommendations).toContain('File Suspicious Activity Report (SAR)')
      expect(results[0].riskLevel).toBe('CRITICAL')
    })
    ...
    test('should detect high frequency trading patterns', () => {
      ...
      const results = complianceSuite.checkCompliance(dayTradingScenario.transactions, 'AML-001')
    ...
    test('should pass compliance for normal trading patterns', () => {
      ...
      const results = complianceSuite.checkCompliance(normalTransactions, 'AML-001')
  ...
  describe('Data Protection Compliance', () => {
    ...
    test('should detect GDPR data retention violations', () => {
      const oldUserData = {
        userId: 'user-old',
        lastLoginDate: new Date(Date.now() - 4 * 365 * 24 * 60 * 60 * 1000).toISOString() // 4 years ago
      }
      ...
      expect(results[0].compliant).toBe(false)
      expect(results[0].violations).toContain(
        expect.stringMatching(/User data retained for \d+ days without activity/)
      )
      expect(results[0].recommendations).toContain(
    ...
    test('should pass compliance for recent user activity', () => {
      ...
      const results = complianceSuite.checkCompliance(activeUserData, 'GDPR-001')
  ...
  describe('Fiduciary Duty Compliance', () => {
    ...
    test('should detect best execution violations', () => {
      const poorExecution = {
        ...
        benchmarkPrice: 170.00, // 5.88% deviation
        executionTime: 90000 // 1.5 minutes
      }
      
      const results = complianceSuite.checkCompliance(poorExecution, 'FIDUCIARY-001')
    ...
    test('should pass compliance for good execution', () => {
      const goodExecution = {
        ...
        benchmarkPrice: 175.00, // 0.29% deviation
        executionTime: 15000 // 15 seconds
      }
      
      const results = complianceSuite.checkCompliance(goodExecution, 'FIDUCIARY-001')

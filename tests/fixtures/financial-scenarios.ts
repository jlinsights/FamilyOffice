...
export const KOREAN_CONSERVATIVE_INVESTOR: FinancialScenario = {
  ...
  description: 'Korean individual with conservative investment approach, focus on large-cap Korean stocks and bonds',
...
export const FAMILY_OFFICE_DIVERSIFIED: FinancialScenario = {
  ...
  description: 'High net worth family office with global diversification across multiple asset classes',
...
export const DAY_TRADER_HIGH_FREQUENCY: FinancialScenario = {
  ...
  description: 'Active day trader with high frequency transactions, primarily in US growth stocks',
...
export const RETIREMENT_INCOME_FOCUSED: FinancialScenario = {
  ...
  description: 'Conservative retirement portfolio focused on dividend income and capital preservation',
...
export const CRYPTO_HEAVY_PORTFOLIO: FinancialScenario = {
  ...
  description: 'High-risk portfolio with significant cryptocurrency exposure and growth stocks',
...
export class FinancialScenarioManager {
  ...
  generateRandomPortfolio(targetValue: number, riskLevel: 'LOW' | 'MEDIUM' | 'HIGH'): FinancialScenario {
    ...
    return {
      ...
      expectedOutcomes: {
        ...
        diversificationScore: riskLevel === 'LOW' ? 90 : riskLevel === 'MEDIUM' ? 70 : 40
      }
    }
  }
  ...
  validateScenario(scenario: FinancialScenario): string[] {
    ...
    const calculatedValue = scenario.positions.reduce((total, pos) => {
      ...
      return total + (pos.currency === 'KRW' ? positionValue / 1300 : positionValue) // Rough USD conversion
    }, 0)

    const valueDiscrepancy = Math.abs(calculatedValue - scenario.portfolioValue) / scenario.portfolioValue
    if (valueDiscrepancy > 0.1) { // 10% tolerance
      errors.push(`Portfolio value mismatch: calculated ${calculatedValue}, expected ${scenario.portfolioValue}`)
    }

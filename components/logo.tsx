...
export const FamilyOfficeLogo: React.FC<LogoProps> = ({
  ...
  const alignmentClasses = align === 'left' ? 'items-start' : 'items-center';

  const taglineAlignmentClasses =
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center ${currentSize.iconSpacing} group`}>
        <h1
          className={`${currentSize.title} font-playfair font-medium ${themeClasses} tracking-tight transition-all duration-200 group-hover:tracking-normal`}
        >
          FamilyOffice
        </h1>
        <div
        ...
        >
          <span
            className={`text-primary-foreground font-medium ${currentSize.iconText} font-playfair`}
            aria-hidden="true"
          >
            S
          </span>
...
export const MinimalFamilyOfficeLogo: React.FC<
...
> = ({ size = 'default', className = '' }) => {
  ...
  return (
    <div
      className={`inline-flex items-center ${currentSize.spacing} group ${className}`}
    >
      <h1
        className={`${currentSize.title} font-playfair font-medium ${themeClasses} tracking-tight transition-all duration-200 group-hover:tracking-normal`}
      >
...
export const PremiumFamilyOfficeLogo: React.FC<
  Omit<LogoProps, 'size' | 'language'>
> = ({ showTagline = true, className = '' }) => {
  const themeClasses = 'text-foreground';
    ...
    >
      ...
      <div className="flex items-center space-x-3 group">
        <div className="relative">
          <h1
            className={`text-2xl font-playfair font-light ${themeClasses} tracking-[-0.01em] relative`}
          >
            FamilyOffice
            {/* 언더라인 효과 */}
        ...
        >
          <span
            className="text-primary-foreground font-semibold text-2xl tracking-wide font-playfair"
            aria-hidden="true"
          >
            S
          </span>

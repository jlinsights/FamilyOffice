...
const governanceItems: Item[] = [
  ...
  {
    id: 'valuation-risk',
    label: '비상장주식 평가 리스크(급등/감액 근거 부족)',
    weight: 1.5,
  },
...
function scoreToPlan(score: number) {
  if (score >= 11)
    return {
      tier: 'Succession Ready',
      color: 'text-blue-700',
      note: '상속·증여 전면 설계 필요(타임라인·증빙 포함)',
    };
  if (score >= 7)
    return {
      tier: 'Value Control',
      color: 'text-emerald-700',
      note: '비상장가치·거버넌스 정비 및 유동성 보강',
    };
  return {
    tier: 'Cash Shield',
    color: 'text-amber-700',
    note: '세후 유동성 우선 확보(법인명의 활용)',
  };
}
...
export function SelfCheckSuccession() {
  ...
  const score = useMemo(() => {
    const sum = [...liquidityItems, ...governanceItems].reduce(
      (acc, i) => acc + (answers[i.id] ? (i.weight ?? 1) : 0),
      0
    );
              ...
              {liquidityItems.map(item => (
                ...
                >
                  <input
                    ...
                    onChange={e =>
                      setAnswers(p => ({ ...p, [item.id]: e.target.checked }))
                    }
                  />

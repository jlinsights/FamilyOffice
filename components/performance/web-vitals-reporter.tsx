...
export default function WebVitalsReporter() {
  ...
  useEffect(() => {
    ...
    const updateData = (data: Partial<WebVitalsData>) => {
      if (data.lcp !== undefined || data.inp !== undefined || data.cls !== undefined) {
        setVitalsData(prev => ({
          ...prev,
          ...data,
          timestamp: Date.now()
  ...
  const getRatingIcon = (rating: string) => {
    switch (rating) {
      ...
      case 'needs-improvement': return <AlertCircle className="w-4 h-4 text-yellow-600" />
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-center py-4">
            <div className="animate-pulse text-gray-600">성능 메트릭을 수집하고 있습니다...</div>
          </div>
        </CardContent>
      </Card>
    )
  ...
  const metrics: VitalMetric[] = [
    ...
    },
    {
      ...
      unit: 'score',
      description: 'Cumulative Layout Shift - 레이아웃 변경으로 인한 시각적 안정성'
    },
    {
      name: 'FCP',
              ...
              <div className="space-y-3">
                ...
                <Badge className={getRatingColor(metric.rating)}>
                  {metric.rating === 'good' ? '좋음' : 
                   metric.rating === 'needs-improvement' ? '개선필요' : '나쁨'}
                </Badge>
                ...
                <div className="text-xs text-gray-600">
                  ...
                  <div className="text-xs text-gray-500">
                    임계값: 좋음 ≤ {formatValue(metric.threshold.good, metric.unit)}, 
                    나쁨 &gt; {formatValue(metric.threshold.poor, metric.unit)}
                  </div>
              </div>
            </CardContent>
          </Card>

...
export function SeoOverviewDashboard() {
  ...
  const avgPosition =
    metrics.length > 0
      ? (
          metrics.reduce((acc, curr) => acc + curr.position, 0) / metrics.length
        ).toFixed(1)
      : '-';
  ...
  const avgChange =
    metrics.length > 0
      ? Math.round(
          metrics.reduce((acc, curr) => acc + curr.change, 0) / metrics.length
        )
      : 0;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Google Search Console 실시간 데이터 기반
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div className="text-center">
            <div
              className={`text-3xl font-bold ${avgChange >= 0 ? 'text-green-600' : 'text-red-600'}`}
            >
              {avgChange > 0 ? '+' : ''}
              {avgChange}%
            </div>
              ...
              {metrics.slice(0, 3).map((m, i) => (
                <li key={i} className="flex items-center justify-between">
                  ...
                  <span className="text-muted-foreground text-xs">
                    {m.position.toFixed(1)}위 / {m.clicks}클릭
                  </span>
                </li>
              ))}
              {metrics.length === 0 && (

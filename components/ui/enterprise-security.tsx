            ...
            {methods.map((method) => (
              <Button
                ...
                className={cn(
                  ...
                  currentMethod === method.id && "border-premium-500 bg-premium-50"
                )}
                onClick={() => handleMethodSelect(method.id as 'sms' | 'email' | 'app')}
              >
          ...
          <div className="space-y-4">
            <div className="text-center">
              <p className="text-sm text-gray-600 mb-4">
                {selectedMethod === 'sms' && '휴대폰으로 인증 코드를 전송했습니다'}
                {selectedMethod === 'email' && '이메일로 인증 코드를 전송했습니다'}
  ...
  return (
    <div className={cn("space-y-6", className)}>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <SecurityScore score={securityStatus.securityScore} recommendations={recommendations} />
        <MFASetup onSetup={onMFASetup} currentMethod={securityStatus.mfaEnabled ? 'app' : undefined} />
      </div>

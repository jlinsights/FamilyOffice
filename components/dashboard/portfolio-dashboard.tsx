...
import { Circle, TrendingUp, TrendingDown, DollarSign, BarChart3, Activity } from 'lucide-react'
...
const mockPortfolioData: PortfolioData = {
  ...
  transactions: [
    { id: '1', type: 'buy', asset: '삼성전자', amount: 5000000, quantity: 100, date: '2024-01-15' },
    { id: '2', type: 'sell', asset: 'SK하이닉스', amount: 3000000, quantity: 50, date: '2024-01-14' },
    { id: '3', type: 'dividend', asset: 'LG화학', amount: 150000, quantity: 0, date: '2024-01-13' }
  ],
  risk: {
    sharpeRatio: 1.2,
    beta: 0.85,
...
export function PortfolioDashboard() {
  ...
  const [data, setData] = useState<PortfolioData | null>(mockPortfolioData)
  ...
  if (!data) {
    return (
      <div className="text-center py-8">
        <p className="text-muted-foreground mb-4">포트폴리오 데이터를 불러올 수 없습니다.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="relative h-48">
            <div className="w-full h-full rounded-full border-8 border-gray-200 dark:border-gray-700 relative">
              {data.map((item, index) => {
                const startAngle = data.slice(0, index).reduce((sum, d) => sum + (d.value / 100) * 360, 0)
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="text-center p-3 bg-muted/50 rounded-lg hover:bg-muted/70 transition-colors">
            ...
            <p className="text-lg font-semibold text-red-600 dark:text-red-400">{data.maxDrawdown}%</p>
          </div>
        </div>
      </div>
    </Card>
  ...
  return (
    <Card className="p-6 hover:shadow-lg transition-all duration-300">
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {data.map((indicator) => (
            <div key={indicator.name} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg hover:bg-muted/70 transition-colors">
              <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={cn(
                  'text-sm font-medium',
                  indicator.change >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                )}>
...
function FinancialMetric({ label, value, change, format, size }: FinancialMetricProps) {

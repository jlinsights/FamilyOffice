...
export const loadTestUtils = {
  ...
  validatePerformanceTargets: (results: LoadTestSummary) => {
    const targets: Record<string, { target: number; actual: number; reverse?: boolean }> = {
      'Portfolio dashboard load time': { target: 2000, actual: results.averageResponseTime },
      'API response time (95th percentile)': { target: 500, actual: results.p95ResponseTime },
      'Error rate': { target: 1, actual: results.errorRate, reverse: true },
    }
    
    return Object.entries(targets).map(([metric, { target, actual, reverse }]) => {

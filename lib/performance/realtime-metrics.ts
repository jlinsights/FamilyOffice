...
class RealTimePerformanceMonitor {
  ...
  private measureNavigationTiming() {
    if (typeof window !== 'undefined' && window.performance) {
      window.addEventListener('load', () => {
        ...
        this.metrics.fcp = performance.getEntriesByName('first-contentful-paint')[0]?.startTime || 0;
  ...
  private reportAPIMetric(url: string, duration: number, status: number, error?: any) {
    ...
    if (duration > 2000 && url.includes('/api/financial')) {
      console.warn(`⚠️ Slow Financial API: ${url} took ${duration.toFixed(2)}ms`);
    }
  }
  ...
  private checkPerformanceBudget() {
    ...
    const violations = Object.entries(budgets)
      .filter(([metric, budget]) => (this.metrics[metric as keyof PerformanceMetrics] || 0) > budget)
      .map(([metric, budget]) => ({
        metric,
        budget,
    ...
    if (violations.length > 0) {
      ...
      violations.forEach(v => {
        console.warn(`${v.metric}: ${v.actual.toFixed(2)}ms > ${v.budget}ms budget`);
      });
      console.groupEnd();
    } else {
...
export const measureAction = (actionName: string, fn: () => void | Promise<void>) => {

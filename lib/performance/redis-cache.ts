...
export class FinancialDataCache {
  ...
  // 다중 주식 데이터 조회 (병렬 처리)
  async getMultipleStockPrices(symbols: string[]): Promise<Record<string, any>> {
  ...
  // 환율 데이터 캐시 (10분 TTL)
  async cacheForexRate(fromCurrency: string, toCurrency: string, data: any): Promise<void> {
...
export class CachePerformanceMonitor {
  ...
  getCacheStats() {
    const total = Object.values(this.metrics).reduce((sum, count) => sum + count, 0)
    const hits = this.metrics.redisHits + this.metrics.browserHits + this.metrics.cdnHits

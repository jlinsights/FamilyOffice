...
export class WebSocketManager extends EventEmitter {
  ...
  private handleDisconnection(clientId: string): void {
    ...
    if (client) {
      ...
      console.log(
        `📱 Client disconnected: ${clientId} (Session: ${sessionDuration}ms)`
      );
...
export class PriceUpdateManager {
  ...
  private processBatch(): void {
    ...
    if (totalSent > 0) {
      console.log(
        `📈 Sent price batch: ${updates.length} symbols to ${totalSent} clients`
      );
    }
  }
...
export class PortfolioUpdateManager {
  ...
  private sendPortfolioUpdate(portfolioId: string, valueData: any): void {
    ...
    if (totalSent > 0) {
      console.log(
        `💼 Portfolio update sent for ${portfolioId} to ${totalSent} clients`
      );
    }
  }

...
// 기본 데이터 타입 검증
export const idSchema = z.string().min(1, 'ID는 필수입니다').max(100, 'ID가 너무 깁니다');
...
export const dateSchema = z.string().regex(
  /^\d{4}-\d{2}-\d{2}$/,
  'YYYY-MM-DD 형식의 날짜를 입력해주세요'
);
export const datetimeSchema = z.string().datetime('올바른 ISO datetime 형식이 아닙니다');

// 페이지네이션 검증
export const paginationSchema = z.object({
  page: z.coerce.number().int().min(1, '페이지는 1 이상이어야 합니다').default(1),
  limit: z.coerce.number().int().min(1).max(100, '한 번에 최대 100개까지 조회 가능합니다').default(20),
...
export const newsletterSubscribeSchema = z.object({
  ...
  tags: z.array(z.string().max(30)).max(10, '태그는 최대 10개까지 설정 가능합니다').default([]),
...
export const stockQuerySchema = z.object({
  symbol: z.string().min(1, '종목 코드는 필수입니다').max(20, '종목 코드가 너무 깁니다'),
  period: z.enum(['1d', '5d', '1mo', '3mo', '6mo', '1y', '2y', '5y']).default('1d'),
...
export const keywordAnalysisSchema = z.object({
  keyword: z.string().min(1, '키워드는 필수입니다').max(200, '키워드가 너무 깁니다'),
  language: z.enum(['ko', 'en']).default('ko'),
  region: z.enum(['KR', 'US', 'JP', 'CN']).default('KR'),
  device: z.enum(['desktop', 'mobile', 'tablet']).default('desktop'),
  competitors: z.array(z.string().url()).max(5, '경쟁사는 최대 5개까지 분석 가능합니다').optional(),
});

export const naverRankingSchema = z.object({
  action: z.enum(['get_rankings', 'add_keyword', 'remove_keyword', 'check_rank']).default('get_rankings'),
...
export const adminSecurityAuditSchema = z.object({
  ...
  eventType: z.enum(['suspicious_login', 'rate_limit_exceeded', 'invalid_auth', 'admin_access', 'data_breach_attempt']).optional(),
...
export const ipUnblockSchema = z.object({
  ...
  reason: z.string().min(10, '차단 해제 사유를 10자 이상 입력해주세요').max(500, '사유가 너무 깁니다'),
});

// Monitoring API 검증
export const monitoringQuerySchema = z.object({
  metric: z.enum(['performance', 'errors', 'usage', 'security']).default('performance'),
...
export const calcomBookingSchema = z.object({
  eventTypeId: z.coerce.number().int().positive('올바른 이벤트 타입 ID가 아닙니다'),
...
export const securityEventSchema = z.object({
  type: z.enum(['suspicious_login', 'rate_limit_exceeded', 'invalid_auth', 'admin_access', 'data_breach_attempt']),
  severity: z.enum(['low', 'medium', 'high', 'critical']),
  description: z.string().min(1, '이벤트 설명은 필수입니다').max(1000, '설명이 너무 깁니다'),
...
export const emailSendSchema = z.object({
  to: emailSchema,
  subject: z.string().min(1, '제목은 필수입니다').max(200, '제목이 너무 깁니다'),
  content: z.string().min(1, '내용은 필수입니다').max(100000, '내용이 너무 깁니다'),

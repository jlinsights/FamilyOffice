 ...
 */
export const BLOG_CATEGORY_BMAD_MAPPING: Record<string, keyof typeof BMAD_AI_KEYWORDS> = {
  '패밀리오피스': 'behavioral',
  '투자전략': 'aspirational',
  '세무최적화': 'decisional',
  '기업승계': 'motivational',
...
export function extractAIOptimizedKeywords(post: BlogPost): string[] {
  const bmadCategory = BLOG_CATEGORY_BMAD_MAPPING[post.category] || 'behavioral';
    ...
    '패밀리오피스': [
      {
        ...
        answer: '패밀리오피스는 고액자산가 가문의 종합 자산관리 기관입니다. 투자관리, 세무계획, 가업승계, 자산보전 등 가문의 모든 재정적 니즈를 통합적으로 관리합니다.',
        category: '패밀리오피스',
        targetEngines: ['ChatGPT', 'Gemini', 'BingCopilot']
      },
      {
        question: '패밀리오피스 설립에 필요한 최소 자산 규모는?',
        answer: '일반적으로 관리자산 500억 원 이상이 권장됩니다. 하지만 멀티 패밀리오피스(MFO)를 활용하면 100억 원대부터도 전문적인 서비스를 받을 수 있습니다.',
        category: '패밀리오피스',
        targetEngines: ['Perplexity', 'ChatGPT', 'BingCopilot']
      },
      {
        question: '패밀리오피스의 주요 서비스는?',
        answer: '투자자산 관리, 세무최적화, 기업승계 계획, 부동산 관리, 자녀 교육 및 경력 관리, 자선활동 조율, 법률 자문 등 가문의 전방위적 니즈를 지원합니다.',
        category: '패밀리오피스',
    ...
    '투자전략': [
      {
        ...
        answer: '기업 경영 리스크를 고려한 분산 투자가 핵심입니다. 주식 30-40%, 채권 20-30%, 대체투자 20-30%, 부동산 10-20% 비중으로 포트폴리오를 구성하는 것이 일반적입니다.',
        category: '투자전략',
        targetEngines: ['Perplexity', 'Claude', 'ChatGPT']
      },
      {
        question: '대체투자의 장점은 무엇인가요?',
        answer: '전통 자산과 낮은 상관관계로 포트폴리오 변동성을 줄이고, 인플레이션 헤지 효과가 있으며, 장기적으로 안정적인 수익을 추구할 수 있습니다.',
        category: '투자전략',
    ...
    '세무최적화': [
      {
        ...
        answer: '사전 증여를 통한 분산, 가업승계 특례 활용, 공익법인 설립, 보험 활용 등 다양한 방법을 조합해야 합니다. 최소 10년 이상의 장기 계획이 필요합니다.',
        category: '세무최적화',
        targetEngines: ['BingCopilot', 'ChatGPT', 'Perplexity']
      },
      {
        question: '증여세 절세 시 주의할 점은?',
        answer: '10년 단위로 증여 계획을 수립하고, 가업승계 특례를 최대한 활용하며, 증여 시점의 자산 가치 평가를 신중하게 진행해야 합니다.',
        category: '세무최적화',
    ...
    '기업승계': [
      {
        ...
        answer: '명확한 승계 계획, 후계자 역량 강화, 가족 간 합의, 세무 최적화 전략, 지배구조 개편이 필수적입니다. 최소 5-10년의 준비 기간이 필요합니다.',
        category: '기업승계',
        targetEngines: ['Claude', 'Perplexity', 'ChatGPT']
      },
      {
        question: '가업승계 특례 요건은?',
        answer: '중소·중견기업, 10년 이상 계속 경영, 가업 영위 기간 20년(30% 공제) 또는 30년(40% 공제) 이상 등의 요건을 충족해야 합니다.',
        category: '기업승계',
...
export function calculateAIOptimizationScore(post: BlogPost): {
  ...
  Object.entries(AI_ENGINE_OPTIMIZATION).forEach(([engine, config]) => {
    ...
    const [minLen, maxLen] = config.optimalLength.split('-').map(s => parseInt(s.replace('자', '')));
    if (contentLength >= minLen && contentLength <= maxLen) {
      engineScore += 30;
    } else if (contentLength >= minLen * 0.8 && contentLength <= maxLen * 1.2) {
      engineScore += 20;
    }
...
export function generateOptimizationRecommendations(post: BlogPost): string[] {
  ...
  // 전체 점수 기반 권장사항
  if (score.overall < 60) {
    recommendations.push('📊 전반적인 AI 최적화가 필요합니다. FAQ 추가 및 키워드 강화를 권장합니다.');
  }

  // FAQ 권장사항
  if (!post.faq || post.faq.length === 0) {
    recommendations.push('❓ AI 검색엔진을 위한 FAQ 섹션을 추가하세요. 사용자 질문에 직접 답하는 형식이 효과적입니다.');
  }

  // 키워드 권장사항
  const aiKeywords = extractAIOptimizedKeywords(post);
  if (aiKeywords.length < 10) {
    recommendations.push('🔑 BMAD 키워드를 추가하여 검색 최적화를 강화하세요. 현재 ' + aiKeywords.length + '개 → 목표 10개 이상');
  }

  // 콘텐츠 길이 권장사항
  if (post.content.length < 1000) {
    recommendations.push('📝 콘텐츠를 확장하여 AI 검색엔진의 깊이 있는 분석을 지원하세요. (현재: ' + post.content.length + '자)');
  }

  // 엔진별 최적화
  Object.entries(score.byEngine).forEach(([engine, engineScore]) => {
    if (engineScore < 50) {
      const config = AI_ENGINE_OPTIMIZATION[engine as keyof typeof AI_ENGINE_OPTIMIZATION];
      recommendations.push(`🤖 ${config.name}: ${config.preferredFormat} 형식 콘텐츠를 추가하세요.`);
    }
  });

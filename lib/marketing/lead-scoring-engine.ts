...
export class LeadScoringEngine {
   ...
   */
  async trackActivity(activity: Omit<LeadActivity, 'score_impact'>): Promise<void> {
    try {
      // 1. 활동 타입별 점수 계산
      ...
      console.log(`✅ 활동 추적 완료: ${activity.activity_type} (${scoreImpact}점)`);
   ...
   * 활동별 점수 계산
   */
  private calculateActivityScore(activityType: string, activityData: Record<string, any>): number {
    ...
    switch (activityType) {
      case 'page_view':
        // 높은 가치 페이지 방문
        if (activityData.page_url && HIGH_VALUE_PAGES.some(page => 
          activityData.page_url.includes(page)
        )) {
      ...
      case 'document_download':
        // 문서 타입별 점수 조정
        if (activityData.document_type === 'whitepaper' || activityData.document_type === 'case_study') {
  ...
  async recalculateLeadScore(contactId: string): Promise<LeadScore> {
    try {
      ...
      console.log(`📊 리드 스코어 업데이트: ${contactId} → ${leadScore.total_score}점 (${leadScore.score_grade})`);
   ...
   * HubSpot에 스코어 동기화
   */
  private async syncScoreToHubSpot(contactId: string, score: number): Promise<void> {
    try {
   ...
   * 육성 워크플로우 트리거
   */
  private async triggerNurtureWorkflow(contactId: string, score: number): Promise<void> {
    try {
  ...
  async trackWebActivity(params: {
    ...
    const isRepeatVisit = await this.checkRepeatVisit(params.contactId, params.pageUrl);
    ...
    // HubSpot에도 웹 활동 기록
    try {
      await this.hubspotClient.trackWebActivity(params.contactId, params.pageUrl, params.pageTitle);
    } catch (error) {
  ...
  private async checkRepeatVisit(contactId: string, pageUrl: string): Promise<boolean> {
    const { data } = await this.supabase
      ...
      .gte('created_at', new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString()) // 24시간 내
      .limit(1);

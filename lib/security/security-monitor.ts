...
export function detectSuspiciousActivity(request: NextRequest): {
  ...
  if (
    suspiciousUserAgents.some(agent => userAgent.toLowerCase().includes(agent))
  ) {
  ...
  if (
    referer &&
    !referer.includes('familyoffices.vip') &&
...
export async function autoSecurityResponse(
  ...
  if (suspiciousActivity.riskLevel === 'high') {
    ...
    console.warn(
      `High-risk activity from IP ${ip}: ${suspiciousActivity.reasons.join(', ')}`
    );
  }
...
export function getSecurityStatus(): SecurityMetrics & {
  ...
  return {
    ...
    recent_events_count:
      securityMetrics.failed_logins + securityMetrics.admin_access_attempts,
...
export async function unblockIP(
  ...
  try {
    ...
    const response = await fetch(
      `${supabaseUrl}/rest/v1/users?id=eq.${adminUserId}&select=email`,
      {
        headers: {
          apikey: supabaseKey,
          Authorization: `Bearer ${supabaseKey}`,

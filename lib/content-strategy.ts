...
export const CONTENT_TEMPLATES: ContentTemplate[] = [
  {
    ...
    seoOptimization: {
      ...
      metaDescriptionTemplate:
        '성공한 기업가를 위한 실무 중심 {주제} 가이드. 즉시 적용 가능한 구체적 방법과 체크리스트 제공.',
  ...
  },
  {
    ...
    seoOptimization: {
      ...
      metaDescriptionTemplate:
        '성장하는 기업가를 위한 {주제} 전략 분석. 전문가 관점의 심층 인사이트와 실행 가능한 로드맵 제공.',
      keywordDensity: 0.02,
  ...
  },
  {
    ...
    seoOptimization: {
      ...
      metaDescriptionTemplate:
        '{날짜} 주간 시장 분석과 투자 기회. 성공한 기업가를 위한 전문적 시장 인사이트와 실행 가능한 투자 전략.',
      keywordDensity: 0.03,
...
export class ContentGenerator {
  ...
  static generateContentStructure(
    ...
    const title = template.seoOptimization.titleFormula.replace(
      '{주제}',
      topic
    );
    const metaDescription =
      template.seoOptimization.metaDescriptionTemplate.replace('{주제}', topic);
   ...
   * BMAD 요소 매핑
   */
  private static getBMDElement(
    section: string,
    bmdElements: Record<string, string>
  ): string | undefined {
    if (section.includes('사례') || section.includes('경험'))
      return bmdElements.behavioral;
    if (section.includes('전략') || section.includes('성장'))
      return bmdElements.motivational;
    if (section.includes('비전') || section.includes('미래'))
      return bmdElements.aspirational;
    if (section.includes('실행') || section.includes('방법'))
      return bmdElements.decisional;
    return undefined;
  }
  ...
  private static getNextPublishDate(templateId: string): Date {
    ...
    const schedule = WEEKLY_CONTENT_SCHEDULE.find(
      s => CONTENT_TEMPLATES.find(t => t.id === templateId)?.type === s.type
    );

    if (!schedule) return new Date(now.getTime() + 24 * 60 * 60 * 1000);

    const targetDay =
      schedule.day === 'tuesday' ? 2 : schedule.day === 'thursday' ? 4 : 5; // friday
...
export class ContentAnalytics {
  ...
  static analyzeContentPerformance(metrics: ContentPerformanceMetrics[]): {
    ...
    const avgEngagement =
      metrics.reduce((sum, m) => sum + (1 - m.bounceRate), 0) / metrics.length;
    const topContent = metrics.sort((a, b) => b.views - a.views)[0];
    const totalConversions = metrics.reduce(
      (sum, m) => sum + m.consultationRequests,
      0
    );
  ...
  private static calculateTrend(
    ...
    switch (type) {
      case 'views':
        ...
        previousAvg =
          previous.reduce((sum, m) => sum + m.views, 0) / previous.length;
        break;
      case 'engagement':
        recentAvg =
          recent.reduce((sum, m) => sum + (1 - m.bounceRate), 0) /
          recent.length;
        previousAvg =
          previous.reduce((sum, m) => sum + (1 - m.bounceRate), 0) /
          previous.length;
        break;
      // 추가 타입들...
    }
  ...
  private static generateRecommendations(
    ...
    if (avgConversionRate < 2) {
      recommendations.push(
        '상담 예약으로의 전환율 개선을 위한 CTA 최적화 필요'
      );
    }
    ...
    if (blogMetrics.length > 0 && newsletterMetrics.length > 0) {
      const blogAvgViews =
        blogMetrics.reduce((sum, m) => sum + m.views, 0) / blogMetrics.length;
      const newsletterAvgOpens =
        newsletterMetrics.reduce((sum, m) => sum + (m.opens || 0), 0) /
        newsletterMetrics.length;
...
export class ContentAutomation {
  ...
  static generateWeeklySchedule(startDate: Date = new Date()): Array<{
    ...
    for (let week = 0; week < 4; week++) {
      WEEKLY_CONTENT_SCHEDULE.forEach(item => {
        ...
        const targetDay =
          item.day === 'tuesday' ? 2 : item.day === 'thursday' ? 4 : 5;
        const daysToAdd =
          ((targetDay - currentDate.getDay() + 7) % 7) + week * 7;
  ...
  private static generateTopicSuggestion(schedule: ContentSchedule): string {
    ...
    const categoryTopics =
      topics[schedule.category as keyof typeof topics] || topics.practical;

...
export class ErrorMonitor {
  ...
  private static async sendToSentry(errorInfo: any) {
    ...
    // Send to Sentry DSN
    await fetch(`https://sentry.io/api/0/projects/${monitoringConfig.sentry.dsn.split('@')[1]}/store/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
  ...
  private static async sendPerformanceToSentry(performanceInfo: any) {
    ...
    const sentryTransaction = {
      ...
      start_timestamp: new Date(performanceInfo.timestamp).getTime() / 1000 - performanceInfo.duration / 1000,
    ...
    await fetch(`https://sentry.io/api/0/projects/${monitoringConfig.sentry.dsn.split('@')[1]}/envelope/`, {
      method: 'POST',
      headers: {
    ...
    if (type === 'error') {
      ...
      payload = {
        series: [{
          ...
          points: [[Math.floor(new Date(data.timestamp).getTime() / 1000), data.duration]],
          type: 'gauge',
          tags: [
...
export async function checkMonitoringHealth(): Promise<{
  ...
  try {
    ...
    if (!response.ok) {
      errors.push(`Internal monitoring API returned status: ${response.status}`)
    }
  } catch (error) {
    services.internal = false
    errors.push(`Internal monitoring API health check failed: ${(error as Error).message}`)
  }
...
export function monitorStartup() {
  if (typeof window !== 'undefined') {
    ...
    window.addEventListener('unhandledrejection', (event) => {
      ErrorMonitor.logError(new Error(`Unhandled Promise Rejection: ${event.reason}`), {
    ...
    process.on('unhandledRejection', (reason, promise) => {
      ErrorMonitor.logError(new Error(`Unhandled Promise Rejection: ${reason}`), {

...
export const FAMILY_OFFICE_AEO_STRATEGY: AEOStrategy = {
  answerFormats: [
    {
      ...
      answer:
        '가업승계는 중소중견기업의 경영권과 소유권을 다음 세대에 안정적으로 이양하는 과정입니다. 세무 최적화, 법적 절차, 후계자 교육을 통해 기업의 지속가능성을 보장하는 종합적인 전략입니다.',
      format: 'paragraph',
    ...
    },
    {
      ...
      answer:
        '중소기업 CEO 절세 방법은 크게 3가지입니다. 첫째, 경영인정기보험 활용으로 연간 최대 1억원 손금처리. 둘째, 가족법인 설립을 통한 소득분산. 셋째, 연구개발비 세액공제 최대 활용입니다.',
      format: 'list',
    ...
    },
    {
      ...
      answer:
        '일반적으로 개인 순자산 30억원 이상 또는 기업 매출 100억원 이상에서 패밀리오피스 서비스의 효과가 극대화됩니다. 자산관리, 세무최적화, 승계계획의 복합적 니즈가 발생하는 구간입니다.',
      format: 'paragraph',
    ...
    },
    {
      ...
      answer:
        '중소기업 정책자금 신청 조건: 1) 중소기업기본법상 중소기업 2) 업력 3년 이상 3) 신용등급 6등급 이상 4) 세금 완납 5) 업종별 특화조건 충족. 자세한 조건은 기관별로 상이합니다.',
      format: 'step_by_step',
  ...
  aiEngineTargets: [
    ...
    },
    {
      ...
      responsePatterns: ['상황 분석 → 한국 기준 해법 → 실무 팁 → 전문가 추천'],
    },
    {
      engine: 'perplexity',
      ...
      responsePatterns: ['현황 분석 → 전문가 의견 → 실증 데이터 → 실행 방안'],
    },
  ],
  ...
  structuredAnswers: [
    {
      ...
      answerTemplate:
        '{업종} 업계 {규모} 기업의 가업승계는 1) 업종 특성 분석 2) 세무 구조 최적화 3) 후계자 준비 4) 법적 절차 완비 순으로 진행됩니다. 특히 {업종}의 경우 {특화포인트}를 주의해야 합니다.',
    ...
    },
    {
      ...
      answerTemplate:
        '{지역} 지역의 정책자금은 1) {지역}테크노파크 2) {지역}신용보증기금 3) {지역}경제진흥원에서 신청 가능합니다. {지역} 특화 지원사업으로 {특화사업명}이 있어 최대 {지원한도}까지 지원받을 수 있습니다.',
...
export class AEOOptimizer {
  // 자연어 질의 최적화
  static optimizeForNaturalLanguage(
    content: string,
    ...
    optimizedContent = optimizedContent
      ...
      .replace(
        /합니다\./g,
        '합니다. 더 자세한 내용이 궁금하시면 언제든 문의해 주세요.'
      );
  ...
  static formatForAIEngine(
    ...
    const engineRules = FAMILY_OFFICE_AEO_STRATEGY.aiEngineTargets.find(
  ...
  static generateVoiceOptimizedContent(topic: string): string {
    ...
    const naturalQuestions = voiceConfig.naturalLanguageQueries.filter(q =>
      q.includes(topic)
    );
  ...
  static generateStructuredAnswer(
    ...
    const template = FAMILY_OFFICE_AEO_STRATEGY.structuredAnswers.find(sa =>
      new RegExp(sa.questionPattern.replace(/\{[^}]+\}/g, '.*')).test(
        questionPattern
      )
    );
  ...
  static analyzeAEOPerformance(
    ...
    const accuracy =
      answers.reduce((acc, a) => acc + a.confidence, 0) / answers.length;

    const conversationalAnswers = answers.filter(
      a =>
        a.answer.includes('입니다') ||
        a.answer.includes('해주세요') ||
        a.answer.includes('도움')
    );
    const conversationalScore =
      (conversationalAnswers.length / answers.length) * 100;

    const aiOptimizedAnswers = answers.filter(
      a => a.format === 'step_by_step' || a.format === 'list'
    );
  ...
  static convertFAQtoAEO(
    ...
    return faqs.map(faq => ({
      ...
      answer: faq.answer,
      format:
        faq.answer.includes('1)') || faq.answer.includes('첫째')
          ? ('list' as const)
          : ('paragraph' as const),
  ...
  private static determineIntent(question: string): AnswerFormat['intent'] {
    if (question.includes('무엇') || question.includes('설명'))
      return 'informational';
    if (question.includes('어떻게') || question.includes('방법'))
      return 'instructional';
    if (question.includes('신청') || question.includes('문의'))

...
export function calculatePerformanceMetrics(
  ...
  const dailyReturn = portfolioReturns.reduce((sum, ret) => sum + ret, 0) / portfolioReturns.length
  ...
  const returns = {
    ...
    yearly: calculateAnnualizedReturn(portfolioValues[0], portfolioValues[portfolioValues.length - 1], totalDays / 365),
    ytd: ((portfolioValues[portfolioValues.length - 1] / portfolioValues[0]) - 1) * 100
  }
  ...
  const sharpeRatio = calculateSharpeRatio(returns.yearly, riskFreeRate * 100, annualizedVolatility)
  const maxDrawdown = calculateMaxDrawdown(portfolioValues) * 100
  const beta = calculateBeta(portfolioReturns, benchmarkReturns)
  const alpha = calculateAlpha(returns.yearly, riskFreeRate * 100, beta, 
    calculateAnnualizedReturn(benchmarkValues[0], benchmarkValues[benchmarkValues.length - 1], totalDays / 365))

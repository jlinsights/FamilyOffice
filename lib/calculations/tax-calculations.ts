...
export function calculateCapitalGains(
  ...
  const holdings = new Map<string, Array<{
    shares: number
    price: number
    date: Date
    fees: number
  ...
  for (const transaction of sortedTransactions) {
    ...
    if (type === 'BUY') {
      ...
      while (remainingShares > 0 && symbolHoldings.length > 0) {
        ...
        const allocatedFees = (holding.fees * sharesToSell) / holding.shares + 
                            (fees * sharesToSell) / shares
        const gain = sellAmount - buyAmount - allocatedFees
        
        // Determine if long-term or short-term
        const daysDiff = Math.floor((sellDate.getTime() - holding.date.getTime()) / (1000 * 60 * 60 * 24))
        ...
        holding.fees -= (holding.fees * sharesToSell) / (holding.shares + sharesToSell)
...
export function calculateTaxLiability(
  ...
  const koreanCapitalGains = calculateCapitalGains(koreanTransactions, holdingPeriod)
  const foreignCapitalGains = calculateCapitalGains(foreignTransactions, holdingPeriod)
  ...
  const localIncomeTax = (totalCapitalGainsTax + totalDividendTax) * KOREAN_TAX_RATES.localIncomeTax
...
export function generateTaxLossHarvesting(
  ...
  for (const position of lossPositions) {
    const unrealizedLoss = (position.averagePrice - position.currentPrice) * position.shares
    const taxRate = position.isKorean ? KOREAN_TAX_RATES.capitalGains.domestic : KOREAN_TAX_RATES.capitalGains.foreign
    const taxSavings = Math.min(unrealizedLoss, remainingGains) * taxRate
    
    opportunities.push({
      symbol: position.symbol,
      unrealizedLoss: Math.round(unrealizedLoss * 100) / 100,
      recommendation: remainingGains > 0 && unrealizedLoss >= 1000 ? 'SELL' : 'HOLD',

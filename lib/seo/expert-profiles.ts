...
export const expertTeam: ExpertProfile[] = [
  ...
  },
  {
    ...
    experience: {
      years: 20,
      companies: ['국세청', '삼일세무법인', '딜로이트 안진', '패밀리오피스 S'],
      achievements: [
        '500+ 세무 컨설팅 성공 사례',
        '평균 40% 절세 효과 달성',
        '국세청 근무 15년 경력',
  ...
  },
  {
    ...
    experience: {
      years: 18,
      companies: ['미래에셋대우', 'KB증권', '한국투자증권', '글로벌자산운용'],
      achievements: [
        '200억원+ 포트폴리오 운용',
        '연평균 15% 수익률 달성',
        '리스크 대비 수익률 업계 1위',
  ...
  },
  {
    ...
    title: '법무 전문가',
    department: '법무컨설팅',
    expertise: ['기업법무', '상속법', '가족신탁', 'M&A 법무', '컴플라이언스'],
    experience: {
      years: 22,
      companies: [
        '김앤장 법률사무소',
...
export function generateEATSignals(expert: ExpertProfile): any {
  return {
    ...
    sameAs: expert.socialProof.linkedinUrl
      ? [expert.socialProof.linkedinUrl]
      : [],
    aggregateRating: {
...
export function calculateTeamEATScore(): {
  ...
  const expertiseScore =
    expertTeam.reduce((sum, expert) => {
      const experienceScore = Math.min(
        (expert.experience.years / 25) * 100,
        100
      );
      const certificationScore = expert.certifications.length * 20;
      const specializationScore = expert.expertise.length * 15;
      return (
        sum + (experienceScore + certificationScore + specializationScore) / 3
      );
    }, 0) / teamSize;
  ...
  const authoritativenessScore =
    expertTeam.reduce((sum, expert) => {
      ...
      return (
        sum + Math.min((publicationScore + mediaScore + awardScore) / 3, 100)
      );
    }, 0) / teamSize;
  ...
  const trustworthinessScore =
    expertTeam.reduce((sum, expert) => {
      ...
      const consultationScore = Math.min(
        expert.socialProof.consultations / 10,
        100
      );
      const socialScore = expert.socialProof.articles * 2;
      return (
        sum +
        Math.min((satisfactionScore + consultationScore + socialScore) / 3, 100)
      );
    }, 0) / teamSize;

  const overallScore =
    (expertiseScore + authoritativenessScore + trustworthinessScore) / 3;
...
export const expertContentMapping = {
  ...
  'expert-legal': {
    ...
    authorPages: [
      '/blog/business-succession-strategy',
...
export function generateTeamStructuredData(): any {
  return {
    ...
    aggregateRating: {
      '@type': 'AggregateRating',
      ratingValue: 4.9,
      reviewCount: expertTeam.reduce(
        (sum, expert) => sum + expert.socialProof.consultations,
        0
      ),
      bestRating: 5,
    },

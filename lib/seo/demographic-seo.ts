...
export function generatePersonalizedKeywords(
  ...
  const motivationalKeys = Object.values(
    bmadKeywordStrategy.motivational
  ).flat();
  const aspirationalKeys = Object.values(
    bmadKeywordStrategy.aspirational
  ).flat();
  const decisionalKeys = Object.values(bmadKeywordStrategy.decisional).flat();

  const industryKeys = agentOSIndustryStrategy[
    target.industry as keyof typeof agentOSIndustryStrategy
  ]
    ? Object.values(
        agentOSIndustryStrategy[
          target.industry as keyof typeof agentOSIndustryStrategy
        ]
      ).flat()
    : [];

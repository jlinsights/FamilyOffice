...
export const CONTENT_KEYWORD_MAPPING: ContentMapping[] = [
  ...
  // 교육 및 프로그램
  {
    ...
    keywords: ['CEO 자산관리 교육', '차세대 경영진 교육', '패밀리오피스 세미나'],
    cluster: 'education',
    contentType: 'service',
    priority: 'medium'
  },
...
export function calculateKeywordPriority(cluster: KeywordCluster): number {
  ...
  const difficultyScore = cluster.difficulty === 'low' ? 1 : cluster.difficulty === 'medium' ? 0.7 : 0.4;
 ...
 * 타겟 키워드 추천
 */
export function getTargetKeywords(contentType: string, limit: number = 5): string[] {
  const relevantMappings = CONTENT_KEYWORD_MAPPING
    .filter(mapping => mapping.contentType === contentType)
    .sort((a, b) => {
 ...
 * 장기 키워드 생성
 */
export function generateLongTailKeywords(primaryKeyword: string, location?: string): string[] {
  const modifiers = [
    '방법', '가이드', '전략', '컨설팅', '서비스', '추천', '비교', '장점', '단점', '비용',
    '절차', '조건', '혜택', '신청', '선택', '종류', '상품', '솔루션', '전문가'
  ];

  const questionWords = [
    '어떻게', '무엇을', '언제', '어디서', '왜', '누가', '얼마나', '몇'
  ];

...
export function optimizeContentForSearchIntent(
  ...
  const agePersonalization =
    ageSpecificContentPersonalization[profile.ageGroup];
  ...
  behavioralKeywords.forEach(keyword => {
    const density = (optimizedContent.match(new RegExp(keyword, 'gi')) || [])
      .length;
    if (density < 2) {
...
function predictEngagement(
  ...
  return Math.round(
    ageWeight * intentWeight * urgencyWeight * lengthScore * 100
  );
}

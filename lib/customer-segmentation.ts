...
export const CUSTOMER_SEGMENTS = {
  'family-office': {
    ...
    description: 'UHNW (Ultra High Net Worth) 고객 전용 프리미엄 패밀리오피스 서비스',
...
// 고객 세그먼트 분류 함수
export function classifyCustomer(netWorth: number, netIncome: number): {
  segment: CustomerSegment;
  serviceInfo: typeof CUSTOMER_SEGMENTS[CustomerSegment];
  qualification: boolean;
} {
  // 패밀리오피스 자격 (순자산 300억+ AND 순이익 5억+)
  if (netWorth >= CUSTOMER_SEGMENTS['family-office'].criteria.netWorth && 
  ...
  if (netWorth >= CUSTOMER_SEGMENTS['fp-center'].criteria.netWorth && 
...
export function generateDynamicContent(segment: CustomerSegment) {
  ...
  return {
    hero: {
      ...
      backgroundImage: segment === 'family-office' ? '/images/ultra-premium-bg.jpg' : 
                      segment === 'fp-center' ? '/images/premium-bg.jpg' : '/images/standard-bg.jpg'
    },
    ...
    ctaButton: {
      ...
      color: segment === 'family-office' ? 'gold' : segment === 'fp-center' ? 'blue' : 'green'
    }
  };
}
      ...
      return [
        {
          ...
          company: 'A그룹',
          netWorth: '1,000억+',
          testimonial: '패밀리오피스를 통해 3세대에 걸친 자산승계 계획을 완성했습니다.'
        }
      ];
    case 'fp-center':
      return [
        {
          ...
          company: 'B테크',
          netWorth: '100억+',
          testimonial: 'FP센터의 전문적인 자산관리로 5년 만에 자산을 두 배로 늘렸습니다.'
        }
      ];
    default:
      return [
        {
          ...
          company: 'C산업',
          netWorth: '30억',
          testimonial: '체계적인 재무상담으로 회사 성장과 함께 개인 자산도 크게 늘었습니다.'
        }
      ];
  }
}

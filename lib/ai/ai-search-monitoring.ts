 ...
 * AI 검색엔진별 콘텐츠 최적화
 */
export function optimizeContentForAI(
  content: string,
  targetEngine: AISearchEngine
): string {
...
export function generateAIOptimizedContent(
  ...
  const getEngine = (platform: string) =>
    AI_SEARCH_ENGINES.find(e => e.platform === platform) as AISearchEngine;

  return {
    chatgptVersion: optimizeContentForAI(baseContent, getEngine('chatgpt')),
    perplexityVersion: optimizeContentForAI(
      baseContent,
      getEngine('perplexity')
    ),
    ...
    bingCopilotVersion: optimizeContentForAI(
      baseContent,
      getEngine('bing-copilot')
    ),
  };
}

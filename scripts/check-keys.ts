...
console.log('------------------------------------------------');
console.log(
  `NEXT_PUBLIC_SUPABASE_URL:      ${url ? 'Exists' : 'MISSING'} (${url})`
);
...
if (service) {
  ...
  if (service === anon) {
    console.error(
      '  WARNING: Service Role Key is IDENTICAL to Anon Key! This is likely wrong.'
    );
  }
}
console.log('------------------------------------------------');
...
console.log(
  `GOOGLE_SERVICE_ACCOUNT_EMAIL:  ${googleEmail ? 'Exists' : 'MISSING'}`
);
console.log(
  `GOOGLE_PRIVATE_KEY:            ${googleKey ? 'Exists' : 'MISSING'}`
);
console.log(
  `SERPER_API_KEY:                ${serperKey ? 'Exists' : 'MISSING'}`
);
console.log('------------------------------------------------');
...
if (googleEmail && googleKey) {
  ...
  try {
    const { GoogleSearchConsoleAPI } =
      await import('../lib/google/search-console');
    ...
    if (startDate && endDate) {
      ...
      if (data.queries.length > 0 && data.queries[0]) {
        console.log(
          `Top Query: ${data.queries[0].query} (${data.queries[0].clicks} clicks)`
        );
      }
    }
  } catch (error: any) {
    ...
    if (error.message.includes('User does not have sufficient permissions')) {
      console.error(
        '  -> Did you add the service account email to Search Console Users?'
      );

...
class InsuranceProductsManager {
  ...
  scanInsuranceProducts() {
    ...
    Object.keys(INSURANCE_CATEGORIES).forEach(categoryKey => {
      ...
      if (fs.existsSync(categoryDir)) {
        productData.categories[categoryKey] = this.scanCategory(
          categoryKey,
          categoryDir
        );

        // 통계 업데이트
        const categoryFileCount = this.getFilesCount(
          productData.categories[categoryKey]
        );
  ...
  scanCategory(categoryKey, categoryDir) {
    ...
    category.providers.forEach(provider => {
      ...
      if (fs.existsSync(providerDir)) {
        categoryData.providers[provider] = this.scanProvider(
          provider,
          providerDir
        );
      }
  ...
  getFilesCount(categoryData) {
    ...
    if (
      categoryData.comparativeAnalysis &&
      Array.isArray(categoryData.comparativeAnalysis)
    ) {
  ...
  generateProductRecommendations(clientProfile) {
    ...
    const { age, income, assets, familyStructure, goals, riskTolerance } =
      clientProfile;

    // 생명보험 추천
    if (goals.includes('inheritance-planning') || assets > 5000000000) {
      // 50억 이상
...
function main() {
  ...
  switch (command) {
    ...
    case 'recommend':
      ...
      const recommendations =
        manager.generateProductRecommendations(testProfile);

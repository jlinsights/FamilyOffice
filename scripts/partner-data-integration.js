...
class PartnerDataManager {
  ...
  scanPartnerData() {
    ...
    Object.keys(PARTNER_DATA_CONFIG).forEach(partnerKey => {
      ...
      if (fs.existsSync(partnerDir)) {
        partnerData[partnerKey] = this.scanPartnerDirectory(partnerKey, partnerDir);
      }
    });
  ...
  generatePartnerDataIndex() {
    ...
    const index = {
      ...
      totalFiles: Object.values(partnerData).reduce((total, data) => 
        total + this.getTotalFileCount(data), 0
      ),
      categories: [...new Set(Object.values(PARTNER_DATA_CONFIG).map(p => p.category))]
    };
  ...
  getTotalFileCount(partnerData) {
    ...
    return Object.values(partnerData.files).reduce((total, files) => 

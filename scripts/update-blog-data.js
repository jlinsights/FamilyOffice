...
function optimizeBlogData() {
  ...
  const blogPostsMatch = blogDataContent.match(
    /export const blogPosts: Record<string, BlogPost> = ({[\s\S]*?});/
  );
  ...
  try {
    ...
    const jsonString = blogPostsString
      .replace(/(\w+):/g, '"$1":')
      .replace(/'/g, '"');
    ...
    const sortedPosts = Object.fromEntries(
      Object.entries(publishedPosts).sort(
        ([_, a], [__, b]) => new Date(b.date) - new Date(a.date)
      )
    );
...
`;
    ...
    console.log(
      `   ⭐ 추천 포스트: ${Object.values(sortedPosts).filter(post => post.featured).length}개`
    );

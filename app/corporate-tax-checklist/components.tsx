...
export const PremiumCard = ({ children, className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <Card className={cn("border-border/50 bg-card/50 backdrop-blur-xl shadow-premium hover:shadow-premium-gold transition-all duration-300", className)} {...props}>
    {children}
  </Card>
);
...
export const SectionHeader = ({ title, subtitle, className, centered = true }: SectionHeaderProps) => (
...
export const StatusBadge = ({ status }: { status: 'critical' | 'high' | 'normal' }) => {
  const styles = {

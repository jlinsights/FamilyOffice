  ...
  try {
    switch (action) {
      case 'schedule-overview':
        // 전체 콘텐츠 스케줄 개요 반환
        const weeklyContent =
          await ContentDistributionSystem.scheduleWeeklyContent();
      ...
      case 'performance-tracking':
        // 콘텐츠 성과 추적 데이터
        const performance =
          await ContentDistributionSystem.trackContentPerformance();
        return NextResponse.json({
          success: true,
          data: {
            ...performance,
            summary: {
              newsletterAvgOpenRate: calculateAvgOpenRate(
  ...
  try {
    ...
    switch (action) {
      case 'schedule-newsletter':
        ...
        const scheduledNewsletter = await scheduleNewsletter(
          day,
          topic,
          customContent
        );

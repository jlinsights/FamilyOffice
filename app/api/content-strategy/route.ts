  ...
  try {
    switch (action) {
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            nextPublishDates: WEEKLY_CONTENT_SCHEDULE.map(schedule => ({
              ...
              day: schedule.day,
              time: schedule.time,
              nextDate: ContentGenerator['getNextPublishDate'](`${schedule.day}-${schedule.type}`)
            }))
          }
        });
      ...
      case 'generate':
        ...
        try {
          const contentStructure = ContentGenerator.generateContentStructure(topic, templateId);
          return NextResponse.json({
            success: true,
            data: {
              topic,
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            schedule: filteredPlan.map(item => ({
              ...
              timeUntilPublish: Math.ceil((item.date.getTime() - Date.now()) / (1000 * 60 * 60 * 24))
            }))
          }
        });
      ...
      case 'analytics-overview':
        ...
        const analytics = ContentAnalytics.analyzeContentPerformance(sampleMetrics);
      ...
      case 'topic-suggestions':
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            seoKeywords: suggestions.map(topic => 
              [`성공한 기업가 ${topic}`, `패밀리오피스 ${topic}`, `CEO ${topic} 전략`]
            ).flat()
          }
        });
      ...
      case 'content-calendar':
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            summary: {
              newsletters: calendar.reduce((sum, day) => 
                sum + day.events.filter(e => e.type === 'newsletter').length, 0
              ),
              blogs: calendar.reduce((sum, day) => 
                sum + day.events.filter(e => e.type === 'blog').length, 0
              )
            }
          }
        });
...
function generateDetailedMetrics() {
  return generateSampleMetrics().map(metric => ({
    ...
    seoScore: Math.round(((20 - (metric.searchRanking['패밀리오피스'] || 10)) / 20) * 100),
    aiVisibilityScore: Object.values(metric.aiMentions).reduce((sum, mentions) => sum + mentions, 0),
...
function generateTopicSuggestions(category: 'practical' | 'strategic' | 'market-analysis'): string[] {
  const topics = {
    practical: [

...
export async function GET(request: NextRequest) {
  try {
    ...
    const metrics = searchParams.get('metrics')?.split(',') || ['traffic', 'conversions', 'keywords'];
    ...
    const filteredData = Object.fromEntries(
      Object.entries(mockAnalyticsData).filter(([key]) => 
    ...
    if (action === 'generate-report') {
      ...
      return NextResponse.json({
        ...
        data: {
          ...
          scheduledDelivery: recipients?.length ? new Date(Date.now() + 5 * 60 * 1000).toISOString() : null,

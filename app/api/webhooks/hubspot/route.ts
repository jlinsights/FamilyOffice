...
async function processHubSpotEvent(supabase: any, event: HubSpotWebhookEvent) {
  ...
  console.log(`Processing HubSpot event: ${eventType} for ${subscriptionType} ${objectId}`);
...
async function handleContactChange(supabase: any, event: HubSpotWebhookEvent) {
  ...
  const importantProperties = [
    'lifecyclestage', 'lead_status', 'lead_score', 
  ...
  if (propertyName === 'lead_score') {
    await handleLeadScoreChange(supabase, objectId.toString(), parseInt(propertyValue || '0'));
  }
 ...
 * 새 콘택트 생성 처리
 */
async function handleContactCreation(supabase: any, event: HubSpotWebhookEvent) {
 ...
 * 리드 스코어 변경 처리
 */
async function handleLeadScoreChange(supabase: any, contactId: string, newScore: number) {
  // 고득점 리드에 대한 자동 액션
  if (newScore >= 80) {
    // 영업팀 알림 (Slack, 이메일 등)
    console.log(`🔥 High-score lead detected: Contact ${contactId} scored ${newScore}`);

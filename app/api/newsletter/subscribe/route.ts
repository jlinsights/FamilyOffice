...
export async function POST(request: NextRequest) {
  try {
    // 입력값 검증
    const validation = await validateApiRequest(request, newsletterSubscribeSchema, 'body');
    
    if (!validation.success) {
      return createValidationErrorResponse(validation);
    }

    const { email, source, tags, utmSource, utmCampaign, utmMedium } = validation.data!;
    ...
    try {
      const subscriberData: any = {
        ...
        utmCampaign: utmCampaign || (source === 'blog' ? 'blog-subscription' : 'footer_signup'),

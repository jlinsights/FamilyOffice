    ...
    const mockAEOData = {
      ...
      conversationalQueries: {
        ...
        followUpOpportunities: [
          {
            query: '가업승계 준비 시기',
            followUps: ['세무 구조 점검', '후계자 교육', '법적 절차'],
          },
          {
            query: '중소기업 절세 방법',
            followUps: ['경영인정기보험', '가족법인', '투자 공제'],
          },
          {
            query: '패밀리오피스 자격',
            followUps: ['자산 진단', '서비스 맞춤화', '비용 상담'],
          },
        ],
      },
      ...
      if (mockAEOData.aiEnginePerformance[engineKey]) {
        responseData = {
          ...
          aiEnginePerformance: {
            chatgpt:
              engine === 'chatgpt'
                ? mockAEOData.aiEnginePerformance.chatgpt
                : mockAEOData.aiEnginePerformance.chatgpt,
            claude:
              engine === 'claude'
                ? mockAEOData.aiEnginePerformance.claude
                : mockAEOData.aiEnginePerformance.chatgpt,
            bard:
              engine === 'bard'
                ? mockAEOData.aiEnginePerformance.bard
                : mockAEOData.aiEnginePerformance.chatgpt,
            hyperclova:
              engine === 'hyperclova'
                ? mockAEOData.aiEnginePerformance.hyperclova
                : mockAEOData.aiEnginePerformance.chatgpt,
            perplexity:
              engine === 'perplexity'
                ? mockAEOData.aiEnginePerformance.perplexity
                : mockAEOData.aiEnginePerformance.chatgpt,
          },
        };
      }
...
function calculateAEOScore(data: any): number {
  ...
  const voiceOptimizationWeight = 0.2;
  const aiEngineWeight = 0.3;
  ...
  const aiEngineScore =
    Object.values(data.aiEnginePerformance).reduce(
      (acc: number, engine: any) => acc + engine.optimizationScore,
      0
...
function generateAEORecommendations(data: any) {
  ...
  // AI 엔진 최적화
  const lowPerformingEngines = Object.entries(data.aiEnginePerformance).filter(
    ([_, performance]: [string, any]) => performance.optimizationScore < 85
  );

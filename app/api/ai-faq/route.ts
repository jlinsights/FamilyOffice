      ...
      case 'platform-optimized':
        ...
        if (!platformFAQs) {
          return NextResponse.json({
            ...
            availablePlatforms: Object.keys(aiOptimizedFAQSystem.platformSpecific)
          }, { status: 400 });
        }
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            optimization: AI_FAQ_OPTIMIZATIONS.find(opt => opt.platform.toLowerCase() === platform)
          }
        });

      case 'bmad-categories':
        // BMAD Method 카테고리별 FAQ
        if (category && !BMAD_FAQ_CATEGORIES[category as keyof typeof BMAD_FAQ_CATEGORIES]) {
          return NextResponse.json({
            success: false,
        ...
        if (category) {
          const categoryData = BMAD_FAQ_CATEGORIES[category as keyof typeof BMAD_FAQ_CATEGORIES];
          return NextResponse.json({
            success: true,
            data: {
              category,
          ...
          return NextResponse.json({
            success: true,
            data: {
              categories: Object.entries(BMAD_FAQ_CATEGORIES).map(([key, value]) => ({
                id: key,
                ...value,
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            metadata: {
              ...
              platforms: Object.keys(aiOptimizedFAQSystem.platformSpecific).length,
...
function generateFAQAnalytics(): any {
  return {
    overview: {
      totalFAQs: aiOptimizedFAQSystem.searchableIndex.length,
      platformVersions: Object.keys(aiOptimizedFAQSystem.platformSpecific).length,
      bmdCategories: Object.keys(BMAD_FAQ_CATEGORIES).length,
    ...
    bmdAnalytics: {
      behavioral: { faqs: 12, avgRating: 4.1, topKeywords: ['실제 사례', '경험'] },
...
function generateFAQStructuredData(limit: number): any {
  ...
  return {
    ...
    mainEntity: faqs.map(faq => ({
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: faq.detailedAnswer || faq.shortAnswer || '전문가 상담을 통해 맞춤 답변을 제공합니다.'
      }
    })),
    about: {
      '@type': 'Thing',

...
export async function POST(request: NextRequest) {
  try {
    ...
    // 3. 요청 데이터 파싱
    const { contact_id, max_recommendations = 5, force_regenerate = false } = await request.json();
...
export async function GET(request: NextRequest) {
  try {
    ...
    const includeEngagement = url.searchParams.get('include_engagement') === 'true';
    ...
    if (includeEngagement && recommendations.length > 0) {
      const totalRecommendations = recommendations.length;
      const viewedRecommendations = recommendations.filter(rec => rec.viewed_at).length;
      const clickedRecommendations = recommendations.filter(rec => rec.clicked_at).length;

      engagementStats = {
        total_recommendations: totalRecommendations,
        viewed_recommendations: viewedRecommendations,
        clicked_recommendations: clickedRecommendations,
        view_rate: totalRecommendations > 0 ? 
          ((viewedRecommendations / totalRecommendations) * 100).toFixed(1) : '0',
        click_rate: totalRecommendations > 0 ? 
          ((clickedRecommendations / totalRecommendations) * 100).toFixed(1) : '0',
      };
    }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    // 2. 요청 데이터 파싱
    const { recommendation_id, engagement_type, additional_data } = await request.json();
      ...
      return NextResponse.json(
        {
          ...
          message: 'engagement_type은 viewed, clicked, downloaded 중 하나여야 합니다.'
        },
        { status: 400 }
      );
    }
    ...
    // 3. 참여도 정보 업데이트
    const contentEngine = getAIContentEngine();
    await contentEngine.trackRecommendationEngagement(recommendation_id, engagement_type);
    ...
    if (additional_data) {
      ...
      if (recommendation) {
        const { getLeadScoringEngine } = await import('@/lib/marketing/lead-scoring-engine');

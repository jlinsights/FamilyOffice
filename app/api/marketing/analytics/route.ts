...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`📊 마케팅 분석 요청: ${daysBack}일간, 상세 정보: ${includeDetails}`);
    ...
    const analytics: any = {
      ...
      // 리드 스코어링 메트릭
      lead_scoring: leadMetrics.status === 'fulfilled' ? {
        total_leads: leadMetrics.value.total_leads,
        qualified_leads: leadMetrics.value.qualified_leads,
        conversion_rate: `${leadMetrics.value.conversion_rate}%`,
        avg_lead_score: parseFloat(leadMetrics.value.avg_lead_score).toFixed(1),
        top_activities: leadMetrics.value.top_activities?.slice(0, 5) || [],
      } : {
        error: 'Failed to fetch lead scoring metrics',
        details: leadMetrics.status === 'rejected' ? leadMetrics.reason?.message : 'Unknown error'
      },

      // 워크플로우 성과
      workflow_performance: workflowMetrics.status === 'fulfilled' ? {
        ...
        details: workflowMetrics.status === 'rejected' ? workflowMetrics.reason?.message : 'Unknown error'
      },

      // 콘텐츠 추천 성과
      content_recommendations: contentMetrics.status === 'fulfilled' ? {
        ...
        top_performing_content: contentMetrics.value.top_performing_content?.slice(0, 3) || [],
      } : {
        error: 'Failed to fetch content recommendation metrics',
        details: contentMetrics.status === 'rejected' ? contentMetrics.reason?.message : 'Unknown error'
      },
      ...
      summary: {
        marketing_health_score: calculateMarketingHealthScore({
          ...
          contentMetrics: contentMetrics.status === 'fulfilled' ? contentMetrics.value : null,
        }),
...
function calculateMarketingHealthScore(metrics: {
  ...
  if (metrics.workflowMetrics) {
    const completionRate = parseFloat(metrics.workflowMetrics.completion_rate) || 0;
...
function generateKeyInsights(metrics: {
  ...
  return insights.length > 0 ? insights : ['충분한 데이터가 수집되면 인사이트를 제공합니다.'];
}
...
function generateRecommendations(metrics: {
  ...
  if (metrics.leadMetrics) {
    ...
    if (conversionRate < 10) {
      recommendations.push('리드 스코어링 기준 재검토 및 질적 리드 확보 전략 수립');
    }
  ...
  if (metrics.contentMetrics) {
    ...
    if (metrics.contentMetrics.top_performing_content?.length > 0) {
      ...
      recommendations.push(`${topContent.content_type} 타입 콘텐츠 확대 및 유사 콘텐츠 제작`);
    }
  }

  return recommendations.length > 0 ? recommendations : ['현재 성과를 유지하며 지속적인 모니터링 진행'];
}
...
async function getDetailedAnalytics(daysBack: number) {
  try {
    ...
    return {
      ...
      lead_score_distribution: analyzeScoreDistribution(scoreDistribution || []),

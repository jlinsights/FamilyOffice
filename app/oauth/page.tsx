  ...
  useEffect(() => {
    const handleOAuthCallback = async () => {
      try {
        ...
        if (result.success) {
          setStatus('success');
          setMessage(result.isNewUser ? '회원가입이 완료되었습니다!' : '로그인이 완료되었습니다!');
          ...
          {process.env.NODE_ENV === 'development' && (
            <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              ...
              <div className="space-y-2 text-xs">
                ...
                <div>
                  <span className="font-medium">Code:</span> {searchParams.get('code') ? '있음' : '없음'}
                </div>
                <div>
                  <span className="font-medium">Error:</span> {searchParams.get('error') || '없음'}
                </div>
                <div>
                  <span className="font-medium">State:</span> {searchParams.get('state') || '없음'}
                </div>
              </div>
            </div>
          )}

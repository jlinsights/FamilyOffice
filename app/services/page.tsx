...
import { CheckCircle, ArrowRight, Building, TrendingUp, Award, Users, Phone, Search, ChevronRight } from "lucide-react"
        ...
        <section className="relative w-full min-h-[90vh] flex flex-col items-center justify-center bg-gradient-to-br from-background via-muted/30 to-background dark:from-background dark:via-muted/10 dark:to-background overflow-hidden pt-20">
          ...
          <div className="relative z-10 text-center max-w-6xl mx-auto px-6">
            ...
            <p className="text-lg md:text-xl text-muted-foreground mb-12 max-w-3xl mx-auto animate-slide-up leading-relaxed" style={{ animationDelay: '300ms' }}>
              <span className="font-semibold text-foreground">법인 지배구조부터 가업승계까지</span> <span className="font-semibold text-primary">전문가 팀의 원스톱 솔루션</span>으로<br />
              기업가의 모든 고민을 해결해드립니다
            </p>
          ...
          <div className="container">
                  ...
                  >
                    <div className="pb-4">
                      <div className="flex items-center mb-4">
                        ...
                        <div>
                          <h3 className="text-xl font-semibold mb-1 text-gray-900 dark:text-white">{category.title}</h3>
                    ...
                    <div>
                      <div className="space-y-3">
                        {category.services.map((service, serviceIndex) => (
                          <div key={serviceIndex} className="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                            <div className="flex-1">
                              <h4 className="font-medium text-sm mb-1 text-gray-900 dark:text-white">{service.title}</h4>
                              <p className="text-xs text-gray-600 dark:text-gray-300 line-clamp-2">{service.description}</p>
                            </div>
                      ...
                      <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                        <p className="text-xs text-gray-600 dark:text-gray-300">
                          총 <span className="font-semibold text-blue-600 dark:text-blue-400">{category.services.length}개</span> 전문 서비스
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700 p-8">
              <div className="text-center mb-8">
                <h3 className="text-2xl font-bold mb-2 text-gray-900 dark:text-white">상세 서비스 안내</h3>
                <p className="text-gray-600 dark:text-gray-300">각 서비스별 세부 내용과 혜택을 확인하세요</p>
              </div>
              ...
              <Tabs defaultValue={SERVICE_CATEGORIES[0]?.id} className="w-full">
                ...
                {SERVICE_CATEGORIES.map((category) => (
                  <TabsContent key={category.id} value={category.id} className="space-y-6">
                    {category.services.map((service, serviceIndex) => (
                      <div key={serviceIndex} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                        <div className="mb-6">
                          <h4 className="text-lg font-semibold mb-2 text-gray-900 dark:text-white">{service.title}</h4>
                          <p className="text-gray-600 dark:text-gray-300">{service.description}</p>
                        </div>
                        <div>
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                              <h5 className="font-semibold mb-3 text-gray-900 dark:text-white">주요 서비스</h5>
                            ...
                            <div>
                              <h5 className="font-semibold mb-3 text-gray-900 dark:text-white">기대 효과</h5>
                              <ul className="space-y-2">
                            ...
                            <div>
                              <h5 className="font-semibold mb-3 text-gray-900 dark:text-white">대상 고객</h5>
                              <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">{service.targetClient}</p>
                              {service.caseStudy && (
                                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                                  <h6 className="font-medium text-sm mb-2 text-gray-900 dark:text-white">성공 사례</h6>
                                  <p className="text-xs text-gray-600 dark:text-gray-300 mb-2">{service.caseStudy.situation}</p>
                                  <p className="text-xs text-blue-600 dark:text-blue-400 font-medium">{service.caseStudy.result}</p>
                                </div>
                              )}
                            </div>
                          </div>

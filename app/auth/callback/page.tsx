  ...
  useEffect(() => {
    const handleAuthCallback = async () => {
      try {
        ...
        if (session?.user) {
          // 사용자 레코드 동기화
          const userRecord = await authService.syncKakaoUserToDatabase(session.user);
          const isNewUser = !userRecord || new Date(userRecord.created_at).getTime() > (Date.now() - 5000); // 5초 이내 생성
          
          const displayInfo = authService.getDisplayUserInfo(session.user, userRecord);
          ...
          setError('인증이 완료되었지만 세션 생성에 실패했습니다. 다시 시도해주세요.');
        }
          ...
          {process.env.NODE_ENV === 'development' && (
            <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              ...
              <div className="space-y-2 text-xs">
                ...
                <div>
                  <span className="font-medium">Access Token:</span> {searchParams.get('access_token') ? '있음' : '없음'}
                </div>
                <div>
                  <span className="font-medium">Refresh Token:</span> {searchParams.get('refresh_token') ? '있음' : '없음'}
                </div>
                <div>
                  <span className="font-medium">Error:</span> {searchParams.get('error') || '없음'}
                </div>
                <div>
                  <span className="font-medium">Token Type:</span> {searchParams.get('token_type') || '없음'}
                </div>
                <div>
                  <span className="font-medium">Expires In:</span> {searchParams.get('expires_in') || '없음'}
                </div>
              </div>
            </div>
          )}

...
const HRLaborManagementPage = () => {
  ...
  const calculateCategoryProgress = (
    category: keyof typeof hrLaborChecklist
  ) => {
  ...
  const getComplianceRisk = () => {
    ...
    if (criticalProgress < 80)
      return {
        ...
        darkBorderColor: 'dark:border-yellow-800/30',
      };
    return {
      level: '안전',
        ...
        <section className="relative py-16 px-4 md:py-24">
          <div className="max-w-6xl mx-auto text-center">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto mb-12">
              ...
              <Card
                className={`${complianceRisk.bgColor} ${complianceRisk.darkBgColor} ${complianceRisk.darkBorderColor} backdrop-blur-sm`}
              >
                <CardHeader className="pb-3">
                  <CardTitle className="text-lg text-card-foreground">
                    컴플라이언스
                ...
                <CardContent>
                  <div
                    className={`text-2xl font-bold ${complianceRisk.color} mb-2`}
                  >
                    {complianceRisk.level}
                  </div>
                  <div className="flex items-center gap-1">
                    {complianceRisk.level.includes('위험') && (
                      <AlertTriangle className="h-5 w-5 text-red-600 dark:text-red-400" />
                    )}
                    {complianceRisk.level === '주의' && (
                      <AlertCircle className="h-5 w-5 text-yellow-600 dark:text-yellow-400" />
                    )}
                    {complianceRisk.level === '안전' && (
              ...
              <TabsContent value="all" className="space-y-6">
                {Object.entries(hrLaborChecklist).map(([key, category]) => {
                  ...
                  const progress = calculateCategoryProgress(
                    key as keyof typeof hrLaborChecklist
                  );
                  const priorityBadge =
                      ...
                      <CardContent className="pt-6">
                        <div className="space-y-3">
                          {category.items.map(item => {
                            const riskLevel =
                              riskLevels[item.risk as keyof typeof riskLevels];
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  >
                                    <div className="flex items-center gap-2 mb-1">
                                      <span
                                        className={`${item.critical ? 'font-medium' : ''} text-card-foreground`}
                                      >
                                        {item.label}
                                      </span>
                                      {item.critical && (
                ...
                <div className="text-center space-y-6">
                  <div>
                    ...
                    <p className="text-lg text-muted-foreground dark:text-gray-300">
                      {criticalProgress < 50 &&
                        '법적 리스크가 높습니다. 즉시 전문가 상담이 필요합니다.'}
                      {criticalProgress >= 50 &&
                        criticalProgress < 80 &&
                        '몇 가지 영역의 개선이 필요합니다.'}
                      {criticalProgress >= 80 &&
                        '지속적인 관리로 법적 리스크를 최소화하고 있습니다.'}
                    </p>
                  </div>
                  ...
                  <Card className="border-red-200 bg-red-50/50 dark:border-red-800/30 dark:bg-red-950/20 backdrop-blur-sm">
                    ...
                    <CardContent>
                      <p className="text-sm mb-4 text-card-foreground/80">
                        근로기준법, 산업안전보건법 등 법적 의무사항 위반 위험이
                        높습니다.
                      </p>
                      <Button
                        variant="destructive"

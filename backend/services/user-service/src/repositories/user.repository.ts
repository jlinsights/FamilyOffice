...
export class UserRepository {
  ...
  async createUser(
    ...
    try {
      ...
      metricsCollector.recordDatabaseOperation('create_user', Date.now() - startTime);
      logger.info('User created successfully', { userId: result.id, tenantId: context.tenantId });
      
      return result;
    } catch (error) {
      logger.error('Failed to create user', { error, tenantId: context.tenantId });
      throw error;
    }
  }

  // 사용자 조회
  async getUserById(userId: string, context: TenantContext): Promise<User | null> {
    const startTime = Date.now();
    ...
    try {
      ...
      metricsCollector.recordDatabaseOperation('get_user_by_id', Date.now() - startTime);
      return user;
    } catch (error) {
      logger.error('Failed to get user by ID', { error, userId, tenantId: context.tenantId });
      throw error;
    }
  }

  // 이메일로 사용자 조회
  async getUserByEmail(email: string, context: TenantContext): Promise<User | null> {
    const startTime = Date.now();
    ...
    try {
      ...
      metricsCollector.recordDatabaseOperation('get_user_by_email', Date.now() - startTime);
      return user;
    } catch (error) {
      logger.error('Failed to get user by email', { error, email, tenantId: context.tenantId });
      throw error;
    }
  }
  ...
  async getUsers(
    ...
    try {
      ...
      metricsCollector.recordDatabaseOperation('get_users', Date.now() - startTime);
      ...
      logger.error('Failed to get users', { error, tenantId: context.tenantId });
      throw error;
    }
  }
  ...
  async updateUser(
    ...
    try {
      ...
      metricsCollector.recordDatabaseOperation('update_user', Date.now() - startTime);
      logger.info('User updated successfully', { userId, tenantId: context.tenantId });
      
      return result;
    } catch (error) {
      logger.error('Failed to update user', { error, userId, tenantId: context.tenantId });
      throw error;
    }
  }
  ...
  async deleteUser(userId: string, context: TenantContext): Promise<void> {
    ...
    try {
      ...
      metricsCollector.recordDatabaseOperation('delete_user', Date.now() - startTime);
      logger.info('User deleted successfully', { userId, tenantId: context.tenantId });
    } catch (error) {
      logger.error('Failed to delete user', { error, userId, tenantId: context.tenantId });
      throw error;
    }
  }
  ...
  async authenticateUser(
    ...
    try {
      ...
      if (user.twoFactorEnabled) {
        ...
        const isValid2FA = await this.verifyTwoFactorCode(user.id, request.twoFactorCode);
        if (!isValid2FA) {
      ...
      metricsCollector.recordDatabaseOperation('authenticate_user', Date.now() - startTime);
      logger.info('User authenticated successfully', { userId: user.id, tenantId: context.tenantId });
      
      return user;
    } catch (error) {
      logger.error('Failed to authenticate user', { error, email: request.email, tenantId: context.tenantId });
      throw error;
    }
  }
  ...
  async createSession(
    ...
    try {
      ...
      metricsCollector.recordDatabaseOperation('create_session', Date.now() - startTime);
      return result.rows[0];
    } catch (error) {
      logger.error('Failed to create session', { error, userId, tenantId: context.tenantId });
      throw error;
    }
  }

  // 세션 조회
  async getSession(sessionId: string, context: TenantContext): Promise<Session | null> {
    const startTime = Date.now();
    ...
    try {
      ...
      metricsCollector.recordDatabaseOperation('get_session', Date.now() - startTime);
      return result.rows[0];
    } catch (error) {
      logger.error('Failed to get session', { error, sessionId, tenantId: context.tenantId });
      throw error;
    }
  }

  // 세션 삭제
  async deleteSession(sessionId: string, context: TenantContext): Promise<void> {
    const startTime = Date.now();
    ...
    try {
      ...
      metricsCollector.recordDatabaseOperation('delete_session', Date.now() - startTime);
      logger.info('Session deleted successfully', { sessionId, tenantId: context.tenantId });
    } catch (error) {
      logger.error('Failed to delete session', { error, sessionId, tenantId: context.tenantId });
      throw error;
    }
  }
  ...
  // 로그인 실패 횟수 증가
  private async incrementLoginAttempts(userId: string, context: TenantContext): Promise<void> {
    await pgPool.query(
  ...
  // 로그인 실패 횟수 초기화
  private async resetLoginAttempts(userId: string, context: TenantContext): Promise<void> {
    await pgPool.query(

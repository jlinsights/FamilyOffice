...
beforeEach(() => {
  ...
  cy.intercept('GET', '/api/financial/stocks/**', {
    fixture: 'stock-data.json',
  }).as('getStockData');
  cy.intercept('GET', '/api/financial/forex/**', {
    fixture: 'forex-data.json',
  }).as('getForexData');
  cy.intercept('GET', '/api/portfolio/**', {
    fixture: 'portfolio-data.json',
  }).as('getPortfolioData');
  cy.intercept('POST', '/api/transactions', {
    fixture: 'transaction-response.json',
...
chai.use((chai, utils) => {
  chai.Assertion.addMethod(
    'financialValue',
    function (expected: number, tolerance: number = 0.01) {
...
Cypress.Commands.add(
  ...
  (metricName: string, maxDuration?: number) => {
    cy.window().then(win => {
      ...
      if (maxDuration && duration > maxDuration) {
        throw new Error(
          `Performance metric '${metricName}' took ${duration}ms, exceeding limit of ${maxDuration}ms`
        );
      }
...
Cypress.Commands.add('validatePortfolioData', (expectedStructure: any) => {
  ...
  cy.get('[data-cy=total-value]').should(
    'contain.text',
    expectedStructure.totalValue || /\$[\d,]+/
  );
  cy.get('[data-cy=day-change]').should('contain.text', /[+-]?\$[\d,]+/);
  cy.get('[data-cy=positions-count]').should(
    'contain.text',
    expectedStructure.positionsCount || /\d+/
  );
});

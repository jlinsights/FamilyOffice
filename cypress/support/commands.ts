...
Cypress.Commands.add('createPortfolio', (portfolioData: {
  name: string
  description?: string
  initialCash?: number
}) => {
  ...
  if (portfolioData.description) {
    cy.get('[data-cy=portfolio-description-input]').type(portfolioData.description)
  }
  
  if (portfolioData.initialCash) {
    cy.get('[data-cy=initial-cash-input]').clear().type(portfolioData.initialCash.toString())
  }
  ...
  cy.get('[data-cy=portfolio-list]').should('contain.text', portfolioData.name)
})

Cypress.Commands.add('addPosition', (positionData: {
  symbol: string
  shares: number
  price: number
}) => {
  ...
  cy.get('[data-cy=positions-table]').should('contain.text', positionData.symbol)
})

// Transaction commands
Cypress.Commands.add('executeTransaction', (transactionData: {
  type: 'BUY' | 'SELL'
  symbol: string
  shares: number
  price?: number
  ...
  cy.get(`[data-cy=transaction-type-${transactionData.type.toLowerCase()}]`).click()
  ...
  if (transactionData.orderType === 'LIMIT' && transactionData.price) {
    ...
    cy.get('[data-cy=limit-price-input]').type(transactionData.price.toString())
  } else {
...
Cypress.Commands.add('generatePerformanceReport', (reportConfig: {
  startDate: string
  endDate: string
  portfolios?: string[]
  ...
  cy.get('[data-cy=report-summary]').should('contain.text', 'Performance Summary')
})
...
Cypress.Commands.add('calculateTaxLiability', (taxYear: number) => {
  ...
  cy.get('[data-cy=tax-calculation-completed]', { timeout: 20000 }).should('be.visible')
...
Cypress.Commands.add('verifyFinancialData', (expectedData: {
  totalValue?: string | RegExp
  dayChange?: string | RegExp
  positionsCount?: number
}) => {
  if (expectedData.totalValue) {
    cy.get('[data-cy=total-portfolio-value]').should('contain.text', expectedData.totalValue)
  }
  
  if (expectedData.dayChange) {
    cy.get('[data-cy=daily-change]').should('contain.text', expectedData.dayChange)
  }
  
  if (expectedData.positionsCount !== undefined) {
    cy.get('[data-cy=position-row]').should('have.length', expectedData.positionsCount)
  }
})
...
Cypress.Commands.add('handleApiError', (expectedErrorMessage?: string) => {
  ...
  if (expectedErrorMessage) {
    cy.get('[data-cy=error-notification]').should('contain.text', expectedErrorMessage)
  }
...
Cypress.Commands.add('waitForRealTimeUpdate', (symbol: string, timeout: number = 15000) => {
...
declare global {
  namespace Cypress {
    interface Chainable {
      ...
      createPortfolio(portfolioData: { name: string; description?: string; initialCash?: number }): Chainable<void>
      addPosition(positionData: { symbol: string; shares: number; price: number }): Chainable<void>

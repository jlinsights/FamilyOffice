...
describe('Transaction Management Workflow', () => {
  ...
  describe('Stock Trading Transactions', () => {
    ...
    it('should execute a buy order successfully', () => {
      ...
      cy.get('[data-cy=total-portfolio-value]').invoke('text').as('initialValue')
      ...
      cy.get('[data-cy=transaction-row]').first().within(() => {
        ...
        cy.get('[data-cy=transaction-price]').should('contain.text', '$185.50')
    ...
    it('should prevent selling more shares than owned', () => {
      ...
      cy.get('[data-cy=validation-error]').should('contain.text', 'Insufficient shares')
    ...
    it('should handle limit order price validation', () => {
      ...
      cy.get('[data-cy=validation-error]').should('contain.text', 'Price must be greater than 0')
      ...
      cy.get('[data-cy=price-warning]').should('contain.text', 'Price is significantly above market')
  ...
  describe('Korean Stock Trading', () => {
    ...
    it('should handle Korean stock transactions with proper formatting', () => {
      ...
      cy.navigateToTransactions()
      cy.get('[data-cy=transaction-row]').first().within(() => {
        cy.get('[data-cy=transaction-symbol]').should('contain.text', '005930.KS')
        cy.get('[data-cy=transaction-price]').should('contain.text', '₩75,000')
        cy.get('[data-cy=transaction-amount]').should('contain.text', '₩7,500,000')
      })
    ...
    it('should handle market hours validation for Korean stocks', () => {
      ...
      cy.get('[data-cy=market-hours-warning]').should('contain.text', 'Korean market is currently closed')
      ...
      cy.navigateToTransactions()
      cy.get('[data-cy=transaction-status]').first().should('contain.text', 'Pending')
    })
  })
  ...
  describe('Transaction History and Search', () => {
    ...
    beforeEach(() => {
      ...
      cy.task('generateTransactionData', {
        ...
        cy.window().then((win) => {
          win.localStorage.setItem('test-transactions', JSON.stringify(transactions))
        })
      })
    })
    ...
    it('should filter transactions by date range', () => {
      ...
      cy.get('[data-cy=transaction-row]').each(($row) => {
        cy.wrap($row).within(() => {
          cy.get('[data-cy=transaction-date]').invoke('text').then((dateText) => {
    ...
    it('should export transaction history', () => {
      ...
      cy.get('[data-cy=export-status]').should('contain.text', 'Export completed')
  ...
  describe('Transaction Reconciliation', () => {
    ...
    it('should detect and flag reconciliation discrepancies', () => {
      ...
      cy.get('[data-cy=file-input]').selectFile('cypress/fixtures/broker-statement.csv')
      cy.get('[data-cy=upload-button]').click()
      
      // Wait for reconciliation process
      cy.get('[data-cy=reconciliation-progress]').should('be.visible')
      cy.get('[data-cy=reconciliation-completed]', { timeout: 20000 }).should('be.visible')
      ...
      if (cy.get('[data-cy=discrepancy-count]').invoke('text').then((text) => parseInt(text) > 0)) {
        cy.get('[data-cy=view-discrepancies]').click()
        cy.get('[data-cy=discrepancy-list]').should('be.visible')
        
        cy.get('[data-cy=discrepancy-item]').first().within(() => {
    ...
    it('should handle manual transaction adjustments', () => {
      ...
      cy.get('[data-cy=adjustment-notes]').type('Corrected price due to broker error')
      ...
      cy.get('[data-cy=audit-trail]').should('contain.text', 'Transaction adjusted')
  ...
  describe('Trading Fees and Tax Calculations', () => {
    ...
    it('should calculate and display trading fees accurately', () => {
      ...
      cy.get('[data-cy=fee-percentage]').invoke('text').then((feeText) => {
    ...
    it('should track tax implications of transactions', () => {
      ...
      cy.get('[data-cy=transaction-with-tax]').first().within(() => {
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('should handle market volatility and price gaps', () => {
      ...
      cy.get('[data-cy=volatility-warning]').should('contain.text', 'High volatility detected')
    ...
    it('should handle network failures gracefully', () => {
      ...
      cy.get('[data-cy=transaction-error]').should('contain.text', 'Transaction failed')
    ...
    it('should handle concurrent transaction conflicts', () => {
      ...
      cy.window().then((win) => {
        ...
        win.localStorage.setItem('updated-position-NVDA', JSON.stringify(currentPosition))
      })
      ...
      cy.get('[data-cy=concurrent-modification-error]').should('contain.text', 'Position has been modified')

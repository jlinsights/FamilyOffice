...
import { getSupabaseKakaoAuthService, type SupabaseKakaoAuthResult } from '@/lib/auth/supabase-kakao-auth';
...
interface UseSupabaseKakaoAuth extends UseSupabaseKakaoAuthState, UseSupabaseKakaoAuthActions {}
...
export function useSupabaseKakaoAuth(): UseSupabaseKakaoAuth {
  ...
  // 사용자 상태 업데이트
  const updateUserState = useCallback((user: User | null, userRecord: UserRecord | null) => {
  ...
  // 카카오 로그인
  const signInWithKakao = useCallback(async (): Promise<SupabaseKakaoAuthResult> => {
  ...
  // 프로필 업데이트
  const updateProfile = useCallback(async (updates: Partial<UserUpdate>): Promise<boolean> => {
    if (!state.user) {
      setError('로그인이 필요합니다.');
      return false;
    }
    ...
    try {
      const success = await authService.updateUserProfile(state.user.id, updates);
  ...
  const unlinkKakaoAccount = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : '카카오 연동 해제 중 오류가 발생했습니다.';
      setError(errorMessage);
      return false;
    }
  }, [state.user, authService, setError]);
  ...
  const refreshUser = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : '사용자 정보를 가져오는 중 오류가 발생했습니다.';
      setError(errorMessage);
    }
  }, [authService, updateUserState, setError]);
  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      try {
        ...
        // 인증 상태 변경 리스너 등록
        const { data: { subscription } } = authService.onAuthStateChange((user, userRecord) => {
          if (mounted) {
            updateUserState(user, userRecord);
          }
        });
        ...
        if (mounted) {
          const errorMessage = error instanceof Error ? error.message : '인증 초기화 중 오류가 발생했습니다.';
          setError(errorMessage);
        }
      }
    };

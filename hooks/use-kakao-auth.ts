...
export function useKakaoAuth(): AuthState & {
  ...
  const unlinkKakaoAccount = useCallback(async (): Promise<{
    success: boolean;
    error?: string;
  }> => {
    setState(prev => ({ ...prev, isLoading: true, error: null }));
  ...
  return {
    ...
    displayName:
      state.kakaoUser?.kakao_account.profile?.nickname ||
      state.user?.user_metadata?.name ||
      '사용자',
    profileImage:
      state.kakaoUser?.kakao_account.profile?.profile_image_url ||

...
export function usePageKeywords(pathname: string): string[] {
  ...
  useEffect(() => {
    ...
    // 현재 경로에 맞는 키워드 설정
    const currentKeywords = pageKeywordMap[pathname] || [
      '패밀리오피스',
      '자산관리',
    ];
...
export function useSEOScore(analysis: ContentAnalysis | null): {
  ...
  const [factors, setFactors] = useState<
    Array<{ name: string; score: number; weight: number; description: string }>
  >([]);
    ...
    const scoringFactors = [
      {
        name: '컨텐츠 길이',
        score:
          analysis.wordCount >= 300 ? 100 : (analysis.wordCount / 300) * 100,
        weight: 0.15,
        description: '최소 300단어 이상 권장',
      },
      {
        name: '키워드 밀도',
        score: Object.values(analysis.keywordDensity).every(
      ...
      },
      {
        name: '내부 링크',
        score:
          analysis.internalLinks.length >= 3
            ? 100
            : (analysis.internalLinks.length / 3) * 100,
        weight: 0.2,
      ...
      },
      {
        ...
        score:
          analysis.readabilityScore >= 60
            ? 100
            : (analysis.readabilityScore / 60) * 100,
        weight: 0.2,
...
const contentOptimizationHooks = {
  useContentOptimization,
  usePageKeywords,
  useSEOScore,
};

export default contentOptimizationHooks;

...
export function useAEOOptimization(options: UseAEOOptimizationOptions = {}) {
  ...
  const [optimizationTasks, setOptimizationTasks] = useState<Array<{
    id: string;
    type: string;
  ...
  const optimizeVoiceSearch = useCallback(async (queries: string[]) => {
    try {
      ...
      // 최적화 작업 추가
      setOptimizationTasks(prev => [...prev, {
  ...
  // AI 엔진 최적화
  const optimizeAIEngines = useCallback(async (engines: string[], contentTypes: string[]) => {
    try {
      const response = await fetch('/api/aeo/analysis', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    try {
      const response = await fetch('/api/aeo/analysis', {
        ...
        body: JSON.stringify({
          ...
          config: { topics, format: 'template', sources: ['expert_knowledge', 'official_docs'] }
        })
      });
  ...
  return {
    ...
    getAEOScore: () => {
      ...
      const voiceScore = data.voiceSearch.voiceReadiness * 0.20;
      const aiEngineScore = Object.values(data.aiEngines).reduce((acc, engine) => 
        acc + engine.optimizationScore, 0) / Object.keys(data.aiEngines).length * 0.30;
      const structuredScore = data.structuredAnswers.accuracyScore * 0.25;
      
      return Math.round(coverageScore + voiceScore + aiEngineScore + structuredScore);
    },
    ...
    getOptimizationProgress: (taskType?: string) => {
      ...
      const avgProgress = filteredTasks.reduce((acc, task) => acc + task.progress, 0) / filteredTasks.length;
      const allCompleted = filteredTasks.every(task => task.status === 'completed');
